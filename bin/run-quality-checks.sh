...
# What the test stage actually ran, which is no longer the same list: a
# global input that moves only the lint step leaves the test result of every
# package intact, so it is re-validated and not re-tested. Recorded as the
# narrower list so the artifact cannot claim a suite that did not run.
_test_packages = data.get("test_packages", data["packages"])
print(json.dumps(_test_packages))
...
'); then
        ...
        TEST_PACKAGES="${_changed_fields[4]}"
        TEST_SCOPE="${_changed_fields[5]}"
