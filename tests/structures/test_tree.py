...
def test_basics():
    ...
    assert ['b', 'c'] == [node.data for node in tree.children]
    assert ['z'] * tree.num_children == [
        node.parent.data
...
def test_collect_some_terminal_nodes():
    ...
    print('collected: {}'.format([node.data for node in some_leaf_nodes]))
...
def test_edges():
    ...
    assert [
        ...
        for (parent, child) in x.get_edges(
                include_self=False, traversal='bfs', as_data=False
        )
    ] == edges
