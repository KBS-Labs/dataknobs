...
def test_sort_by_str_length():
    ...
    assert pd_utils.sort_by_strlen(
        df, 'text', ascending=True
    )['text'].tolist() == ['a', 'abc', 'abcd', 'abcdef']
    assert pd_utils.sort_by_strlen(
        df, 'text', ascending=False
    )['text'].tolist() == ['abcdef', 'abcd', 'abc', 'a']
...
def test_group_manager_subgroups():
    df = pd.DataFrame({
        ...
        'date_field': ['day', 'month', 'day', 'month', 'year', 'day', 'month', 'day', 'month', 'year'],
    })
    g_num = pd_utils.GroupManager(df, 'date_num')
    g_num.mark_groups([
        [0, 3, 4], [1, 2, 4], [5, 8, 9], [6, 7, 9],
    ])
    g_rec = pd_utils.GroupManager(g_num.collapsed_df, 'date_recsnum')
    g_rec.mark_groups([
        [0, 3, 4, 5, 8, 9], [1, 2, 4, 6, 7, 9],
    ])
