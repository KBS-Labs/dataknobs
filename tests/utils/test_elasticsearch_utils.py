...
def test_build_field_query_dict_basic_withop():
    ...
    expected = {
        'query': {
            'match': {'data': {'query': 'this is a test', 'operator': 'AND'}}
        }
    }
    assert fqd == expected
...
def test_build_field_query_dict_single_multifield():
    ...
    expected = {
        'query': {
            'match': {'data': {'query': 'this is a test', 'operator': 'OR'}}
        }
    }
    assert fqd == expected
...
def test_build_field_query_dict_multifield():
    expected = {
        'query': {
            'multi_match': {'query': 'this is a test', 'fields': ['data1', 'data2']}
        }
    }
...
def test_build_phrase_query_dict():
    ...
    expected = {
        'query': {
            'match_phrase': {'data': {'query': 'this is a test', 'slop': 2}}
        }
    }
    assert pqd == expected
...
def test_batchfile_functions():
    ...
    assert recs.equals(pd.DataFrame([
        {'data': n, 'id': n}
        for n in range(0, 10)
    ]))
...
def build_mock_requests():
    ...
    mock_requests.add(
        requests_utils.MockResponse(200, {'foo': 'bar'}),
        'post', 'http://localhost:9200/_close',
    )
    mock_requests.add(
        requests_utils.MockResponse(200, {'foo': 'bar'}),
    ...
    mock_requests.add(
        requests_utils.MockResponse(200, {'foo': 'bar'}),
        'post', 'http://localhost:9200/_open',
    )
    mock_requests.add(
        requests_utils.MockResponse(200, {'foo': 'bar'}),
        'put', 'http://localhost:9200/_mapping',
    ...
    mock_requests.add(
        requests_utils.MockResponse(200, {'foo': 'bar'}),
        'delete', 'http://localhost:9200/test-data',
        headers=requests_utils.HEADERS,
    )
    ...
    eidx = es_utils.ElasticsearchIndex(
        ...
        [
            es_utils.TableSettings(
                'test-data', {'settings': None}, {'properties': None}
            )
        ],
