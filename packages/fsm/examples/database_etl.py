...
def validate_records(state) -> Dict[str, Any]:
    ...
    for record in data.get("extracted_records", []):
        ...
        if errors:
            validation_errors.append({"record_id": record.get("id"), "errors": errors})
...
def transform_records(state) -> Dict[str, Any]:
    ...
    for record in data.get("validated_records", []):
        try:
            ...
            transformed["total_amount"] = round(record["quantity"] * record["unit_price"], 2)
...
def finalize_etl(state) -> Dict[str, Any]:
    ...
    data["etl_summary"] = {
        ...
        "success_rate": (
            data["statistics"]["records_loaded"] / data["statistics"]["records_extracted"] * 100
            if data["statistics"]["records_extracted"] > 0
            else 0
        ),
    ...
    "networks": [
        {
            ...
            "states": [
                ...
                {
                    "name": "initialize",
                    "functions": {"transform": {"type": "registered", "name": "initialize_etl"}},
                },
                {
                    "name": "extract",
                    "functions": {"transform": {"type": "registered", "name": "extract_data"}},
                },
                {
                    "name": "validate",
                    "functions": {"transform": {"type": "registered", "name": "validate_records"}},
                },
                {
                    "name": "transform",
                    "functions": {"transform": {"type": "registered", "name": "transform_records"}},
                },
                {
                    "name": "staging",
                    "functions": {"transform": {"type": "registered", "name": "load_to_staging"}},
                },
                {
                    "name": "commit",
                    "functions": {"transform": {"type": "registered", "name": "commit_to_target"}},
                },
                {
                    "name": "rollback",
                    "functions": {"transform": {"type": "registered", "name": "rollback_staging"}},
                },
                {
                    "name": "finalize",
                    "functions": {"transform": {"type": "registered", "name": "finalize_etl"}},
                },
            ...
            "arcs": [
                ...
                {"from": "extract", "to": "validate"},
                {
                    "from": "validate",
                    "to": "transform",
                    "condition": {"type": "registered", "name": "check_validation_passed"},
                },
                {
                    "from": "validate",
                    "to": "rollback",
                ...
                {"from": "transform", "to": "staging"},
                {
                    "from": "staging",
                    "to": "commit",
                    "condition": {"type": "registered", "name": "check_ready_to_commit"},
                },
                {
                    "from": "staging",
                    "to": "rollback",
                ...
                {"from": "rollback", "to": "finalize"},
                {
                    "from": "finalize",
                    "to": "success",
                    "condition": {"type": "inline", "code": "data.get('committed', False)"},
                },
                {
                    "from": "finalize",
                    "to": "failure",
                    "condition": {"type": "inline", "code": "not data.get('committed', False)"},
                },
            ],
        }
    ],
...
def simulate_database_operations():
    ...
    source_db.executemany("INSERT INTO sales_raw VALUES (?,?,?,?,?,?,?,?,?)", sample_data)
    source_db.commit()
    ...
    with create_etl_fsm() as fsm:
        ...
        with SimpleFSM(
            ...
            result = fsm_with_failures.process(
                {"source_table": "sales_raw", "target_table": "sales_fact", "batch_size": 100}
            )
