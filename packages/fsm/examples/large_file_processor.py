...
def initialize_processing(state) -> Dict[str, Any]:
    ...
    if not file_ref:
        ...
        data["processing"] = {
            ...
            "statistics": {"min_value": None, "max_value": None, "sum": 0, "count": 0},
        }
        return data
...
def process_chunk(state) -> Dict[str, Any]:
    ...
    chunk_stats = {"lines_processed": 0, "lines_failed": 0, "chunk_errors": []}
    ...
    if chunk_stats["chunk_errors"]:
        data["processing"]["errors"].extend(
            chunk_stats["chunk_errors"][:10]
    ...
    "networks": [
        {
            ...
            "states": [
                {"name": "start", "is_start": True},
                {
                    "name": "initialize",
                    "functions": {
                        "transform": {"type": "registered", "name": "initialize_processing"}
                    },
                },
                {
                ...
                {
                    "name": "aggregate",
                    "functions": {"transform": {"type": "registered", "name": "aggregate_results"}},
                },
                {
                    "name": "success",
                    "is_end": True,
                    "functions": {"transform": {"type": "registered", "name": "mark_success"}},
                },
                {
                    "name": "failure",
                    "is_end": True,
                    "functions": {"transform": {"type": "registered", "name": "mark_failure"}},
                },
            ],
            "arcs": [
                {"from": "start", "to": "initialize"},
                {
                    "from": "initialize",
                    "to": "process_chunks",
                    "condition": {"type": "registered", "name": "check_initialization_success"},
                },
                {
                    "from": "initialize",
                    "to": "failure",
                ...
                {"from": "process_chunks", "to": "aggregate"},
                {
                    "from": "aggregate",
                    "to": "success",
                    "condition": {"type": "registered", "name": "check_processing_complete"},
                },
                {
                    "from": "aggregate",
                    "to": "failure",
    ...
    with SimpleFSM(
        ...
        for file_type in file_types:
            ...
            with tempfile.NamedTemporaryFile(
                mode="w", suffix=f".{file_type if file_type != 'text' else 'txt'}", delete=False
            ) as tmp_file:
            ...
            try:
                ...
                result = fsm.process(
                    {"file_reference": str(tmp_path), "chunk_data": chunks[0] if chunks else []}
                )
