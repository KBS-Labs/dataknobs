...
> **Historical design note.** The strategy-based transaction coordinator
> (`core/transactions.py`, `TransactionStrategy`) checked off below was never
> wired to drive database commit/rollback and has since been removed. Database
> atomicity is provided by the dataknobs-data transaction primitives
> (`AsyncDatabase.transaction()`, the `DatabaseTransaction` function,
> `BatchCommit(atomicity="require")`). The checklist is retained for historical
> context only.
...
- API refactoring completed (10-api-to-general-refactor.md) ✅
...
- Identified API code duplication issues that need refactoring (documented in 10-api-to-general-refactor.md)
...
- [ ] **Example Implementations** (`examples/`) **[PARTIALLY COMPLETE]**
  ...
  - [x] Advanced FSM Debugger ✅
    ...
    - [x] **Note**: Depends on AdvancedFSM improvements (see docs/active/16-advanced-fsm-improvements.md) ✅
...
- [x] **Documentation** **[MOSTLY COMPLETE]**
  ...
  - [ ] Migration Guide
  - [x] Custom Functions Design Document (15-custom-functions-design-fix.md) ✅
  - [x] Consolidated FSM Design (14-consolidated-fsm-design.md) ✅
  - [x] Advanced FSM Improvements Plan (16-advanced-fsm-improvements.md) ✅
