...
- [x] **Example Implementations** (`examples/`) **[MOSTLY COMPLETE]**
...
**Next Task**: Implement LLM conversation system example
...
- [ ] **Simplify FSM data handling**
  - Root cause: FSM wraps data in SimpleNamespace in some paths
  - Functions receive inconsistent data formats (dict vs SimpleNamespace)
  - Need to standardize data passing throughout FSM
  - Remove need for type checking in user functions
  - Options to consider:
    1. Always pass raw dict data to functions
    2. Always pass wrapped data with consistent interface
    3. Make wrapping behavior configurable per FSM/state
  - Current workaround requires all functions to check data type
- [ ] **Enhance LLM example**
  - Add Ollama integration as LLM backend option
  - Support for actual LLM API calls (currently mocked)
...
- **Solution Options**:
  1. Always pass raw dict data to functions (simplest for users)
  2. Always pass wrapped data with consistent interface that supports both access patterns
  3. Make wrapping behavior configurable per FSM/state
- **Current Workaround**: All transform functions must check data type and handle both cases
...
### Categories of Failures After Refactor
1. **State Transition Failures** (~10 tests)
   - Tests expecting specific state transitions that aren't happening
   - Example: `test_conditional_branching` expects 'process' state but gets 'validate' or 'error'
   - Likely cause: Transform functions not returning expected values due to data wrapping

2. **Data Access Failures** (~15 tests)
   - KeyError when accessing data fields (e.g., `KeyError: 'count'`, `KeyError: 'processed'`)
   - TypeError with SimpleNamespace/StateNamespace subscript access
   - Likely cause: Data wrapping inconsistency between dict and namespace objects

3. **Transform Execution Failures** (~8 tests)
   - Transforms not being executed or returning unexpected results
   - Example: `test_state_transform_vs_arc_transform_separation` expecting 2 transforms but getting 1
   - Likely cause: Interface wrapper not being called correctly

4. **Validation/Error Path Failures** (~7 tests)
   - Tests expecting error states but getting success states
   - Example: `test_validation_with_routing` expects 'invalid_end' but gets 'valid_end'
   - Likely cause: Validation functions not working with wrapped data
