...
class NetworkSelector:
    ...
    def get_current_network(
        fsm: FSM, context: ExecutionContext, enable_intelligent_selection: bool = True
    ) -> StateNetwork | None:
        ...
        if context.network_stack:
            ...
            if hasattr(context, "metadata"):
                context.metadata["network_selection_warning"] = (
                    f"Network '{network_name}' not found in stack"
                )
    ...
    def score_arc(
        ...
        if include_resource_check and hasattr(arc, "resources") and arc.resources:
            ...
            resources_available = all(
                res in context.resources and context.resources[res].status == "available"
...
class TransitionSelector:
    ...
    def _select_by_strategy(
        ...
        if strategy == TraversalStrategy.DEPTH_FIRST:
            ...
            for arc in available:
                resource_count = (
                    len(arc.resource_requirements) if hasattr(arc, "resource_requirements") else 0
                )
