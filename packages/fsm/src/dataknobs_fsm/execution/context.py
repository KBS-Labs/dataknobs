...
class ExecutionContext:
    ...
    def __init__(
        ...
        self.current_state_owned_resources: Dict[
            str, Any
        ] = {}  # Subset of current_state_resources this state acquired itself (released on exit; inherited ones are left to the ancestor that owns them)
        self.parent_state_resources: Dict[
            str, Any
        ] = {}  # Resources from parent state (in subnetworks)
    ...
    def allocate_resource(
        self, resource_type: str, resource_id: str, metadata: Dict[str, Any] | None = None
    ) -> bool:
        """Allocate a resource.
    ...
    def get_resource_usage(self) -> Dict[str, Any]:
        ...
        """
        allocated = sum(1 for r in self.resources.values() if r.status == ResourceStatus.ALLOCATED)
        busy = sum(1 for r in self.resources.values() if r.status == ResourceStatus.BUSY)
