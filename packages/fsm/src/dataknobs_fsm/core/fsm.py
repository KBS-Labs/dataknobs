...
class FSM:
    ...
    def add_network(self, network: StateNetwork, is_main: bool = False) -> None:
    ...
    def find_state_definition(
        self, state_name: str, network_name: str | None = None
    ...
    def create_state_instance(
        self, state_name: str, data: Dict[str, Any] | None = None, network_name: str | None = None
    ) -> StateInstance:
        ...
        if not state_def:
            ...
            state_def = StateDefinition(
                ...
                if state_name in ["start", "Start", "START"]
                else StateType.NORMAL,
            )

        # Create and return state instance
        return StateInstance(definition=state_def, data=data or {})
    ...
    def _prepare_execution_context(self, initial_data: Dict[str, Any] | None = None):
        ...
        context = ExecutionContext(data_mode=self.data_mode, transaction_mode=self.transaction_mode)
    ...
    async def execute_async(self, initial_data: Dict[str, Any] | None = None) -> Any:
        ...
        try:
            ...
            success, result = await engine.execute(
                context, initial_data if self.data_mode == ProcessingMode.SINGLE else None
            )
