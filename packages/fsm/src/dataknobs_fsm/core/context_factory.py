...
class ContextFactory:
    ...
    def create_context(
        ...
        context = ExecutionContext(
            data_mode=data_mode, transaction_mode=transaction_mode, resources=resources or {}
        )
        ...
        state_name, state_def = ContextFactory._resolve_initial_state(fsm, initial_state)
    ...
    def _resolve_initial_state(
        fsm: FSM, state_name: str | None = None
        ...
        if not state_def and state_name:
            state_def = StateDefinition(
                name=state_name, type=StateType.START if state_name == "start" else StateType.NORMAL
            )
    ...
    def create_batch_context(
        ...
        """
        context = ExecutionContext(data_mode=data_mode, resources=resources or {})
        ...
        if state_def:
            context.current_state_instance = StateInstance(definition=state_def, data={})

        return context

    @staticmethod
    def create_stream_context(
        fsm: FSM, stream_context, resources: Dict[str, Any] | None = None
    ) -> ExecutionContext:
