...
class FileStreamSource(IStreamSource):
    ...
    def read_chunk(self) -> StreamChunk | None:
        ...
        try:
            ...
            # Return error chunk
            return StreamChunk(
                data=[], sequence_number=self._chunk_count, metadata={"error": str(e)}, is_last=True
            )
...
class FileStreamSink(IStreamSink):
    ...
    def _open_file(self) -> None:
        ...
        if self.atomic and not self.append:
            self._temp_path = self.file_path.with_suffix(self.file_path.suffix + ".tmp")
        ...
        mode = (
            "ab"
            if self.append and self.format == FileFormat.BINARY
            else ("a" if self.append else "w")
        )
        ...
        if self.compression == CompressionFormat.GZIP:
            if self.format == FileFormat.BINARY:
                ...
                self._file_handle = gzip.open(str(target_path), mode + "t", encoding=self.encoding)
        elif self.format == FileFormat.BINARY:
...
class DirectoryStreamSource(IStreamSource):
    ...
    def read_chunk(self) -> StreamChunk | None:
        ...
        while self._current_file_index < len(self.files):
            ...
            if self._current_source is None:
                ...
                try:
                    self._current_source = FileStreamSource(
                        file_path, format=self.format, chunk_size=self.chunk_size
                    )
                except Exception:
            ...
            chunk.metadata["source_file"] = str(self.files[self._current_file_index])
            chunk.metadata["file_index"] = self._current_file_index
            chunk.metadata["total_files"] = len(self.files)

            # Update is_last flag
            chunk.is_last = self._current_file_index == len(self.files) - 1 and chunk.is_last
