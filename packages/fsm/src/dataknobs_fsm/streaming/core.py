...
class StreamContext(StructuredConfigConsumer[StreamConfig]):
    ...
    def add_processor(self, processor: Callable[[StreamChunk], StreamChunk | None]) -> None:
    ...
    def _check_backpressure(self) -> None:
        ...
        if (
            input_size > self.config.backpressure_threshold
            or output_size > self.config.backpressure_threshold
        ):
    ...
    def stream(
        ...
        for i in range(self.config.parallelism):
            worker = threading.Thread(target=self._worker_thread, name=f"stream-worker-{i}")
            worker.daemon = True
            worker.start()
                ...
                try:
                    ...
                    if success:
                        ...
                        self.processed_records += (
                            len(chunk.data) if hasattr(chunk.data, "__len__") else 1
                        )
                    else:
