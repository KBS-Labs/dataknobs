...
class Bulkhead(StructuredConfigConsumer[BulkheadConfig]):
    ...
    def _setup(self) -> None:
        ...
        self.queued_count = 0
        self.metrics = (
            {"executed": 0, "rejected": 0, "timeout": 0} if config.track_metrics else None
        )
        ...
        try:
            ...
            if self.config.fallback_config:
                ...
                if config.fallback_on_exceptions:
                    if not any(
                        isinstance(e, exc_type) for exc_type in config.fallback_on_exceptions
                    ):
                        raise
...
def create_retry_workflow(
    ...
    config = ErrorRecoveryConfig(
        ...
        retry_config=RetryConfig(
            max_attempts=max_attempts, backoff_strategy=backoff_strategy, **kwargs
        ),
    )
...
def create_circuit_breaker_workflow(
    ...
    config = ErrorRecoveryConfig(
        ...
        circuit_breaker_config=CircuitBreakerConfig(
            failure_threshold=failure_threshold, timeout=timeout, **kwargs
        ),
    )
