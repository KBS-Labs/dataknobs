...
class APIOrchestrator(StructuredConfigConsumer[APIOrchestrationConfig]):
    ...
    def _build_fsm(self) -> SimpleFSM:
        ...
        if self.config.mode == OrchestrationMode.SEQUENTIAL:
            ...
            for i, endpoint in enumerate(self.config.endpoints):
                ...
                if i == 0:
                    arcs.append(
                        {"from": "start", "to": state_name, "name": f"init_{endpoint.name}"}
                    )
                ...
                if i < len(self.config.endpoints) - 1:
                    ...
                    arcs.append(
                        {"from": state_name, "to": "end", "name": f"{endpoint.name}_complete"}
                    )
    ...
    async def orchestrate(self, input_data: Dict[str, Any] | None = None) -> Dict[str, Any]:
...
def create_rest_api_orchestrator(
    ...
    config = APIOrchestrationConfig(
        endpoints=api_endpoints, mode=mode, global_rate_limit=rate_limit, metrics_enabled=True
    )
...
def create_graphql_orchestrator(
    ...
    if batch_queries:
        ...
        endpoint_config = APIEndpoint(
            name="graphql_batch", url=endpoint, method="POST", headers=headers, body=batched_query
        )

        config = APIOrchestrationConfig(
            endpoints=[endpoint_config], mode=OrchestrationMode.SEQUENTIAL
        )
    else:
        ...
        for q in queries:
            endpoint_config = APIEndpoint(
                ...
                method="POST",
                headers=headers,
                body={"query": q["query"], "variables": q.get("variables", {})},
            )
            api_endpoints.append(endpoint_config)

        config = APIOrchestrationConfig(endpoints=api_endpoints, mode=OrchestrationMode.PARALLEL)
