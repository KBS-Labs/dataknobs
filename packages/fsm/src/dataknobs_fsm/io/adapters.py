...
class AsyncFileProvider(AsyncIOProvider):
    ...
    async def open(self) -> None:
        ...
        self.file_handle = await aiofiles.open(
            self.config.source, mode=mode, encoding=self.config.encoding
        )
        self._is_open = True
...
class AsyncDatabaseProvider(AsyncIOProvider):
    ...
    async def batch_write(
        self, batches: AsyncIterator[List[Any]], table: str = None, **kwargs
    ) -> None:
...
class SyncDatabaseProvider(SyncIOProvider):
    ...
    def batch_read(
        self, query: str = None, batch_size: int | None = None, **kwargs
...
class AsyncHTTPProvider(AsyncIOProvider):
    ...
    async def open(self) -> None:
        ...
        self.session = aiohttp.ClientSession(
            headers=self.config.headers, timeout=aiohttp.ClientTimeout(total=self.config.timeout)
        )
        self._is_open = True
    ...
    async def write(self, data: Any, **kwargs) -> None:
        ...
        async with self.session.post(self.config.source, data=json_data, **kwargs) as response:
            response.raise_for_status()
    ...
    async def _chunked_file_upload(self, data_stream: AsyncIterator[Any], **kwargs) -> None:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                response.raise_for_status()
                # Read, then discarded. `stream_write` is `-> None` on the
                # abstract base and in all seven adapters, so a response body
                # returned from here has nowhere to go -- `stream_write` drops
                # it, and `chunked_upload_from_file` then documents a "Response
                # from the server" it structurally cannot deliver. Kept as a
                # read because draining the body is what releases the
                # connection; only the `return` is gone.
                _ = (
                    await response.json()
    ...
    async def chunked_upload_from_file(
        self, file_path: str, chunk_size: int = 8192, **kwargs
    ) -> Any:
...
class SyncHTTPProvider(SyncIOProvider):
        ...
        try:
            if self.session:
                response = self.session.head(  # type: ignore[unreachable]
                    self.config.source, timeout=self.config.timeout or 30
                )
    ...
    def read(self, **kwargs) -> Any:
        ...
        if not self.session:
            self.open()
        response = self.session.get(self.config.source, timeout=self.config.timeout or 30, **kwargs)
        response.raise_for_status()
    ...
    def write(self, data: Any, **kwargs) -> None:
        ...
        response = self.session.post(
            self.config.source, data=json_data, timeout=self.config.timeout or 30, **kwargs
        )
        response.raise_for_status()
    ...
    def stream_read(self, **kwargs) -> Iterator[Any]:
        ...
        response = self.session.get(
            self.config.source, stream=True, timeout=self.config.timeout or 30, **kwargs
        )
        response.raise_for_status()
