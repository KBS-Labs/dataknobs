    ...
    def _transform_simple_to_network(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "states" in config and "networks" not in config:
            ...
            network_config = {
                ...
                "networks": [
                    {"name": "main", "states": states, "arcs": self._add_type_to_transforms(arcs)}
                ],
                "main_network": "main",
            }
            ...
            if "data_mode" in config:
                ...
                if isinstance(mode, str):
                    ...
                    network_config["data_mode"] = {
                        "default": mode.lower()
                        if mode.lower() in ["copy", "reference", "direct"]
                        else "copy"
                    }
                else:
                        ...
                        if "from" in arc and "to" in arc:
                            ...
                            if "pre_test" in arc and "condition" not in arc:
                                ...
                                if isinstance(pre_test, dict) and "test" in pre_test:
                                    ...
                                    state_arc["condition"] = self._convert_to_function_reference(
                                        pre_test["test"]
                                    )
                                    ...
                                    if field == "name":
                                        ...
                                        if isinstance(condition, dict):
                                            ...
                                            if condition == "success":
                                                ...
                                                state_arc["condition"] = (
                                                    self._convert_to_function_reference(condition)
                                                )
                                        else:
                                            state_arc[field] = condition
                                    else:
                    ...
                    for state in network["states"]:
                        ...
                        if "functions" in state and isinstance(state["functions"], dict):
                            ...
                            if "validate" in functions:
                                ...
                                state["validators"] = [
                                    self._convert_to_function_reference(validate_func)
                                ]
                            ...
                            if "transform" in functions:
                                ...
                                state["transforms"] = [
                                    self._convert_to_function_reference(transform_func)
                                ]
                        ...
                        if "transform" in state and "transforms" not in state:
                            ...
                            if isinstance(transform, list):
                                ...
                                state["transforms"] = [
                                    self._convert_to_function_reference(transform)
                                ]
                            del state["transform"]
                        ...
                        if "validator" in state and "validators" not in state:
                            ...
                            if isinstance(validator, list):
                                ...
                                state["validators"] = [
                                    self._convert_to_function_reference(validator)
                                ]
                            del state["validator"]
