...
class TestStateTransformExecution:
    ...
    def test_state_transform_executed_once_per_state_entry(self):
        ...
        try:
            with SimpleFSM(
                config, custom_functions={"counting_transform": counting_transform}
            ) as fsm:
                result = fsm.process({"value": 42})

                # Verify the transform was called exactly once
                assert call_count == 1, (
                    f"Transform should be called exactly once, but was called {call_count} times"
                )
    ...
    def test_state_transform_with_chained_transformations(self):
        ...
        try:
            with SimpleFSM(
                ...
                assert result["path"] == [
                    "start",
                    "first_transform_state",
                    "second_transform_state",
                    "end",
                ]
            ...
            "networks": [
                {
                    ...
                    "states": [
                        ...
                        {
                            "name": "transform_state",
                            "functions": {"transform": "state_transform"},
                            "arcs": [
                                {
                                    "target": "end",
                                    "transform": {"type": "inline", "code": "arc_transform"},
                                }
                            ],
                        },
                        {"name": "end", "is_end": True},
                    ],
                    "arcs": [{"from": "start", "to": "transform_state"}],
                }
            ],
        }
    ...
    def test_state_transform_not_executed_on_state_validation(self):
            ...
            "networks": [
                {
                    ...
                    "states": [
                        {"name": "start", "is_start": True},
                        {
                            ...
                            "validators": [{"type": "inline", "code": "test_validator"}],
                            "transforms": [{"type": "inline", "code": "test_transform"}],
                        },
                        {"name": "end", "is_end": True},
                    ],
                    "arcs": [
        ...
        try:
            with SimpleFSM(
                ...
                assert transform_call_count == 1, (
                    f"Transform should be called exactly once, got {transform_call_count}"
                )
