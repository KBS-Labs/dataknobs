...
class TestSyncDatabaseProvider:
    ...
    def test_provider_initialization(self):
        ...
        config = IOConfig(source="test.db", format="sqlite", mode=IOMode.WRITE)
    ...
    def test_provider_open_file_database(self):
        ...
        try:
            config = IOConfig(mode=IOMode.WRITE, format="sqlite", source=db_path)
...
class TestSyncHTTPProvider:
    ...
    def test_provider_initialization(self):
        ...
        config = IOConfig(source="http://api.test.com/data", format="json", mode=IOMode.READ)
    ...
    def test_provider_read_json(self, mock_session_class):
        ...
        assert provider._is_open is True
        mock_session.get.assert_called_once_with("http://api.test.com/data", timeout=30)
    ...
    def test_provider_stream_read(self, mock_session_class):
        ...
        mock_session.get.assert_called_once_with(
            "http://api.test.com/stream", stream=True, timeout=30
        )
    ...
    def test_provider_stream_read_text(self, mock_session_class):
        ...
        mock_response.iter_lines.return_value = [b"plain text line 1", b"plain text line 2"]
        mock_session.get.return_value = mock_response
        mock_session_class.return_value = mock_session
    ...
    def test_provider_batch_operations(self, mock_session_class):
        ...
        config = IOConfig(
            mode=IOMode.WRITE, format="json", source="http://api.test.com/batch", batch_size=2
        )
        ...
        mock_responses = [
            Mock(
                ...
                json=Mock(return_value=[{"id": 0}, {"id": 1}]),
            ),
            Mock(
                headers={"content-type": "application/json"},
                json=Mock(return_value=[{"id": 2}, {"id": 3}]),
            ),
