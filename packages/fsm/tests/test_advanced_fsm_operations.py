...
class TestAdvancedFSMOperations:
            ...
            "networks": [
                {
                    ...
                    "states": [
                        {"name": "start", "is_start": True},
                        {
                            "name": "initialize",
                            "functions": {
                                "transform": {"type": "registered", "name": "initialize_workflow"}
                            },
                        },
                        {
                            "name": "validate",
                            "functions": {
                                "transform": {"type": "registered", "name": "validate_input"}
                            },
                        },
                        {
                            "name": "process",
                            "functions": {
                                "transform": {"type": "registered", "name": "process_action"}
                            },
                        },
                        {
                            "name": "finalize",
                            "functions": {
                                "transform": {"type": "registered", "name": "finalize_workflow"}
                            },
                        },
    ...
    def test_inspect_state(self, workflow_config, custom_functions):
        ...
        fsm = create_advanced_fsm(workflow_config, custom_functions=custom_functions)
    ...
    def test_set_hooks(self, workflow_config, custom_functions):
        ...
        # Track hook calls
        hook_calls = {"state_enter": [], "state_exit": [], "error": []}
    ...
    async def test_trace_execution(self, workflow_config, custom_functions):
        ...
        fsm = create_advanced_fsm(
            workflow_config, custom_functions=custom_functions, execution_mode=ExecutionMode.TRACE
        )

        # Test data with valid input
        test_data = {"user_id": "USER-123", "action": "create", "data": {"name": "Test Item"}}
        ...
        trace_path = [
            f"{t.get('from', t.get('from_state'))} → {t.get('to', t.get('to_state'))}"
            for t in trace
        ]
    ...
    async def test_profile_execution(self, workflow_config, custom_functions):
        ...
        fsm = create_advanced_fsm(
            workflow_config, custom_functions=custom_functions, execution_mode=ExecutionMode.PROFILE
        )
    ...
    async def test_different_actions(self, workflow_config, custom_functions):
        ...
        # Test create action
        create_data = {"user_id": "USER-123", "action": "create", "data": {"name": "New Item"}}
        ...
        # Test delete action
        delete_data = {"user_id": "USER-123", "action": "delete", "id": 789}
