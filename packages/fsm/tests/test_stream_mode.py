...
class TestStreamMode:
    ...
    def create_stream_fsm_config(self) -> FSMConfig:
        ...
        network = NetworkConfig(
            ...
            states=[
                StateConfig(
                    name="start", is_start=True, arcs=[ArcConfig(target="process", priority=1)]
                ),
                StateConfig(
                    name="process",
                    functions={
    ...
    async def test_stream_mode_multiple_chunks(self):
        ...
        context = ExecutionContext(
            data_mode=ProcessingMode.STREAM, transaction_mode=fsm.transaction_mode
        )
    ...
    async def test_stream_mode_with_batch_chunks(self):
        ...
        context.stream_context.add_chunk(
            StreamChunk(data=chunk1_data, chunk_id="batch_1", is_last=False)
        )

        context.stream_context.add_chunk(
            StreamChunk(data=chunk2_data, chunk_id="batch_2", is_last=False)
        )

        context.stream_context.add_chunk(
            StreamChunk(data=chunk3_data, chunk_id="batch_3", is_last=True)
        )
                    ...
                    arcs=[
                        ArcConfig(
                            ...
                            condition=FunctionReference(
                                type="inline", code="data.get('id', 0) % 2 != 0"
                            ),
                        )
                    ],
                ),
                StateConfig(name="process", arcs=[ArcConfig(target="end", priority=1)]),
                StateConfig(name="end", is_end=True),
            ],
        )

        config = FSMConfig(
            name="failing_stream_fsm", networks=[network], main_network="failing_stream"
        )
    ...
    async def test_stream_mode_performance(self):
        ...
        for i in range(num_chunks):
            ...
            context.stream_context.add_data({"chunk_id": i, "data": f"chunk_{i}"}, is_last=is_last)
