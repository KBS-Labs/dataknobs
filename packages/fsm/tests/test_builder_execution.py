...
class TestFSMExecute:
    ...
    def create_simple_fsm_config(self, name: str = "test_fsm") -> FSMConfig:
        ...
        network = NetworkConfig(
            ...
            states=[
                StateConfig(
                    name="start", is_start=True, arcs=[ArcConfig(target="end", priority=1)]
                ),
                StateConfig(name="end", is_end=True),
            ],
        )

        return FSMConfig(name=name, description="Test FSM", networks=[network], main_network="main")
                            ...
                            arcs=[
                                ArcConfig(
                                    ...
                                    transform=FunctionReference(
                                        type="inline", code="data['error'] = True; data"
                                    ),
                                )
                            ],
                        ),
    ...
    async def test_execute_with_complex_data(self, fsm_instance):
        ...
        complex_data = {
            "nested": {"list": [1, 2, 3], "dict": {"key": "value"}, "tuple": (4, 5, 6)},
            "array": [{"id": 1}, {"id": 2}],
        }
    ...
    async def test_execute_with_complex_fsm_config(self):
        ...
        network = NetworkConfig(
            ...
            states=[
                StateConfig(
                    name="init", is_start=True, arcs=[ArcConfig(target="process", priority=1)]
                ),
                StateConfig(name="process", arcs=[ArcConfig(target="validate", priority=1)]),
                StateConfig(
                    name="validate",
                    arcs=[
