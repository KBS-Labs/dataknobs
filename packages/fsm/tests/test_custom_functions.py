...
class TestCustomFunctionRegistration:
    ...
    def test_resolve_registered_function(self):
        ...
        func_ref = FunctionReference(type="registered", name="my_custom_func")
    ...
    def test_resolve_unregistered_function_fails(self):
        ...
        func_ref = FunctionReference(type="registered", name="nonexistent_func")
...
class TestSimpleFSMCustomFunctions:
            ...
            "networks": [
                {
                    ...
                    "states": [
                        {"name": "start", "is_start": True},
                        {
                            "name": "transform",
                            "functions": {
                                "transform": {"type": "registered", "name": "custom_transform"}
                            },
                        },
    ...
    def test_simple_fsm_with_custom_functions(self, simple_config):
        ...
        with SimpleFSM(
            simple_config, custom_functions={"custom_transform": custom_transform}
        ) as fsm:
    ...
    def test_multiple_custom_functions(self):
            ...
            "networks": [
                {
                    ...
                    "states": [
                        {"name": "start", "is_start": True},
                        {
                            "name": "first",
                            "functions": {
                                "transform": {"type": "registered", "name": "first_func"}
                            },
                        },
                        {
                            "name": "second",
                            "functions": {
                                "transform": {"type": "registered", "name": "second_func"}
                            },
                        },
                        {"name": "end", "is_end": True},
                    ],
        ...
        with SimpleFSM(
            config, custom_functions={"first_func": first_func, "second_func": second_func}
        ) as fsm:
            result = fsm.process({})
...
class TestValidationPipeline:
    ...
    def test_validation_with_routing(self):
            ...
            "networks": [
                {
                    ...
                    "states": [
                        {"name": "start", "is_start": True},
                        {
                            "name": "validate",
                            "functions": {
                                "transform": {"type": "registered", "name": "validate_data"}
                            },
                        },
                    ...
                    "arcs": [
                        {"from": "start", "to": "validate"},
                        {
                            "from": "validate",
                            "to": "valid_end",
                            "condition": {"type": "inline", "code": "data.get('is_valid', False)"},
                        },
                        {
                            "from": "validate",
                            "to": "invalid_end",
        ...
        def validate_data(state):
            ...
            data["validation_message"] = (
                "Valid" if data["is_valid"] else "Invalid: value must be positive"
            )
            return data

        with SimpleFSM(config, custom_functions={"validate_data": validate_data}) as fsm:
            # Test valid data
    ...
    def test_multi_field_validation(self):
            ...
            "networks": [
                {
                    ...
                    "states": [
                        {"name": "start", "is_start": True},
                        {
                            "name": "validate_all",
                            "functions": {
                                "transform": {"type": "registered", "name": "validate_fields"}
                            },
                        },
                        {"name": "end", "is_end": True},
                    ],
        ...
        def validate_fields(state):
            ...
            data["all_valid"] = all(v["valid"] for v in data["validations"].values())

            return data

        with SimpleFSM(config, custom_functions={"validate_fields": validate_fields}) as fsm:
...
class TestCustomFunctionErrors:
            ...
            "networks": [
                {
                    ...
                    "states": [
                        {"name": "start", "is_start": True},
                        {
                            "name": "transform",
                            "functions": {
                                "transform": {"type": "registered", "name": "missing_func"}
                            },
                        },
                        {"name": "end", "is_end": True},
                    ],
    ...
    def test_state_transform_exception_handling(self):
        ...
        with SimpleFSM(
            config, custom_functions={"failing_state_transform": failing_state_transform}
        ) as fsm:
            result = fsm.process({})
    ...
    def test_arc_transform_exception_handling(self):
                    ...
                    "arcs": [
                        {
                            "from": "start",
                            "to": "middle",
                            "transform": {"type": "registered", "name": "failing_arc_transform"},
                        },
                        {"from": "middle", "to": "end"},
        ...
        with SimpleFSM(
            config, custom_functions={"failing_arc_transform": failing_arc_transform}
        ) as fsm:
            result = fsm.process({})
