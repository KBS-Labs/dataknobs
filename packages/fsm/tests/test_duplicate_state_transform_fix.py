...
class TestDuplicateStateTransformFix:
    ...
    def test_state_transform_not_executed_twice(self):
            ...
            "networks": [
                {
                    ...
                    "states": [
                        ...
                        {"name": "multiply", "functions": {"transform": "tracking_transform"}},
        ...
        with SimpleFSM(config, custom_functions={"tracking_transform": tracking_transform}) as fsm:
            ...
            assert result["data"] == {"result": 10}, (
                f"Final result should be {{'result': 10}} (5 * 2), but got {result['data']}"
            )
            ...
            "networks": [
                {
                    ...
                    "states": [
                        ...
                        {"name": "doubler", "functions": {"transform": "value_doubler"}},
                        {"name": "end", "is_end": True},
                    ],
                    "arcs": [{"from": "start", "to": "doubler"}, {"from": "doubler", "to": "end"}],
                }
            ],
        }

        # Create FSM with custom function
        with SimpleFSM(config, custom_functions={"value_doubler": value_doubler}) as fsm:
            ...
            for i, (input_data, expected_output) in enumerate(test_cases):
                ...
                assert result["data"] == expected_output, (
                    f"Test case {i}: Expected {expected_output}, got {result['data']}"
                )
            ...
            "networks": [
                {
                    ...
                    "states": [
                        {"name": "start", "is_start": True},
                        {"name": "transform1", "functions": {"transform": "first_transform"}},
                        {"name": "transform2", "functions": {"transform": "second_transform"}},
                        {"name": "transform3", "functions": {"transform": "third_transform"}},
                        {"name": "end", "is_end": True},
                    ],
                    "arcs": [
