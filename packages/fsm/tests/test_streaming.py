...
class TestStreamCore:
    ...
    def test_stream_chunk_creation(self):
        ...
        data = [1, 2, 3, 4, 5]
        chunk = StreamChunk(data=data, sequence_number=1, metadata={"test": "value"}, is_last=False)
    ...
    def test_stream_context_basic(self):
        ...
        context = StreamContext(StreamConfig(chunk_size=10, parallelism=2))
...
class TestFileStreaming:
    ...
    def test_file_sink_json(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Write chunks
            chunk1 = StreamChunk(data=[{"id": 1}, {"id": 2}], sequence_number=0)
            chunk2 = StreamChunk(data=[{"id": 3}], sequence_number=1, is_last=True)
    ...
    def test_file_sink_jsonl(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            chunk = StreamChunk(data=[{"id": 1}, {"id": 2}, {"id": 3}], is_last=True)
    ...
    def test_file_sink_csv(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            chunk = StreamChunk(
                data=[{"id": 1, "name": "Item1"}, {"id": 2, "name": "Item2"}], is_last=True
            )
    ...
    def test_directory_stream_source(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            source = DirectoryStreamSource(temp_path, pattern="*.json", chunk_size=2)
...
class TestDatabaseStreaming:
    ...
    def test_database_stream_source(self):
        ...
        source = DatabaseStreamSource(database=db, batch_size=2)
    ...
    def test_database_stream_sink(self):
        ...
        # Create sink
        sink = DatabaseStreamSink(database=db, batch_size=2)

        # Write chunks
        chunk1 = StreamChunk(data=[{"id": "1", "value": "a"}, {"id": "2", "value": "b"}])
        chunk2 = StreamChunk(data=[{"id": "3", "value": "c"}], is_last=True)
