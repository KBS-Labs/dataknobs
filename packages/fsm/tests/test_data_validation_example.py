...
class TestDataValidationPipeline:
    ...
    def test_all_valid_data(self, fsm):
        ...
        result = fsm.process({"email": "john.doe@gmail.com", "phone": "(555) 123-4567", "age": 25})
    ...
    def test_invalid_email_domain(self, fsm):
        ...
        result = fsm.process({"email": "user@invalid.com", "phone": "5551234567", "age": 30})
        ...
        assert any(
            "Domain 'invalid.com' is not in allowed list" in error
            for error in result["data"]["validation_errors"]
        )

    def test_invalid_phone(self, fsm):
        """Test with invalid phone number."""
        result = fsm.process({"email": "user@yahoo.com", "phone": "123", "age": 22})
        ...
        assert any(
            "Phone must be 10 digits, got 3" in error
            for error in result["data"]["validation_errors"]
        )

    def test_underage(self, fsm):
        """Test with underage user."""
        result = fsm.process({"email": "teen@gmail.com", "phone": "5551234567", "age": 16})
        ...
        assert any(
            "Must be 18 or older, got 16" in error for error in result["data"]["validation_errors"]
        )

    def test_multiple_issues(self, fsm):
        """Test with multiple validation issues."""
        result = fsm.process({"email": "not-an-email", "phone": "abc", "age": -5})
    ...
    def test_missing_fields(self, fsm):
        ...
        assert any(
            "Phone number is required" in error for error in result["data"]["validation_errors"]
        )
    ...
    def test_validation_path(self, fsm):
        ...
        result = fsm.process({"email": "test@gmail.com", "phone": "5551234567", "age": 25})
    ...
    def test_partial_validation_path(self, fsm):
        ...
        result = fsm.process({"email": "bad-email", "phone": "5551234567", "age": 25})
    ...
    def test_data_normalization(self, fsm):
        ...
        result = fsm.process({"email": "  JOHN@GMAIL.COM  ", "phone": "(555) 123-4567", "age": 30})
    ...
    def test_age_boundary_values(self, fsm):
        ...
        result = fsm.process({"email": "user@gmail.com", "phone": "5551234567", "age": 18})
        assert result["data"]["validations"]["age"]["valid"] is True

        # Test just below minimum (17)
        result = fsm.process({"email": "user@gmail.com", "phone": "5551234567", "age": 17})
        assert result["data"]["validations"]["age"]["valid"] is False

        # Test maximum reasonable age (150)
        result = fsm.process({"email": "user@gmail.com", "phone": "5551234567", "age": 150})
        assert result["data"]["validations"]["age"]["valid"] is True

        # Test just above maximum (151)
        result = fsm.process({"email": "user@gmail.com", "phone": "5551234567", "age": 151})
        assert result["data"]["validations"]["age"]["valid"] is False

    def test_validation_score_calculation(self, fsm):
        """Test that validation score is correctly calculated."""
        # 0/3 valid
        result = fsm.process({"email": "bad", "phone": "bad", "age": -1})
        assert result["data"]["validation_score"] == 0.0

        # 1/3 valid
        result = fsm.process({"email": "user@gmail.com", "phone": "bad", "age": -1})
        assert result["data"]["validation_score"] == 1 / 3

        # 2/3 valid
        result = fsm.process({"email": "user@gmail.com", "phone": "5551234567", "age": -1})
        assert result["data"]["validation_score"] == 2 / 3

        # 3/3 valid
        result = fsm.process({"email": "user@gmail.com", "phone": "5551234567", "age": 25})
        assert result["data"]["validation_score"] == 1.0
