...
class TestAsyncFileProvider:
    ...
    async def test_file_operations(self):
        ...
        try:
            # Test read
            config = IOConfig(mode=IOMode.READ, format=IOFormat.JSON, source=temp_path)
    ...
    async def test_stream_read(self):
        ...
        try:
            config = IOConfig(mode=IOMode.STREAM, format=IOFormat.JSON, source=temp_path)
            provider = AsyncFileProvider(config)
    ...
    async def test_batch_read(self):
        ...
        try:
            config = IOConfig(
                mode=IOMode.BATCH, format=IOFormat.JSON, source=temp_path, batch_size=3
            )
            provider = AsyncFileProvider(config)
...
class TestIOUtils:
    ...
    def test_create_io_provider(self):
        ...
        # File provider
        config = IOConfig(mode=IOMode.READ, format=IOFormat.JSON, source="/path/to/file.json")
        provider = create_io_provider(config, is_async=True)
        assert isinstance(provider, AsyncFileProvider)

        # API provider (HTTP)
        config = IOConfig(
            mode=IOMode.READ, format=IOFormat.API, source="https://api.example.com/data"
        )
        provider = create_io_provider(config, is_async=True)
    ...
    async def test_retry_io_operation(self):
        ...
        result = await retry_io_operation(
            failing_operation, max_retries=3, delay=0.01, exceptions=(ValueError,)
        )
    ...
    async def test_retry_io_operation_failure(self):
        ...
        with pytest.raises(ValueError, match="Permanent error"):
            await retry_io_operation(
                always_failing, max_retries=2, delay=0.01, exceptions=(ValueError,)
            )
