...
class TestStateNetwork:
    ...
    def test_add_arc(self):
        ...
        arc2 = network.add_arc("s2", "s3", transform="process_data", metadata={"priority": 1})
...
class TestArcExecution:
    ...
    async def test_can_execute_with_pretest(self):
        ...
        arc_def1 = ArcDefinition(target_state="next", pre_test="always_true")
        ...
        arc_def2 = ArcDefinition(target_state="next", pre_test="always_false")
    ...
    async def test_can_execute_missing_function(self):
        ...
        arc_def = ArcDefinition(target_state="next", pre_test="nonexistent")
        arc_exec = ArcExecution(arc_def, "current", self.function_registry)
    ...
    async def test_execute_with_transform(self):
        ...
        arc_def = ArcDefinition(target_state="next", transform="double_value")
        arc_exec = ArcExecution(arc_def, "current", self.function_registry)
    ...
    async def test_execute_with_failure(self):
        ...
        arc_def = ArcDefinition(target_state="next", transform="fail_function")
        arc_exec = ArcExecution(arc_def, "current", self.function_registry)
