...
class TestConfigCommands:
    ...
    def test_config_create_basic(self, runner):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = runner.invoke(cli, ["config", "create", "basic", "--output", str(output_path)])
    ...
    def test_config_create_etl_template(self, runner):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = runner.invoke(cli, ["config", "create", "etl", "--output", str(output_path)])
    ...
    def test_config_create_json_format(self, runner):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = runner.invoke(
                cli,
                ["config", "create", "workflow", "--output", str(output_path), "--format", "json"],
            )
    ...
    def test_config_validate_verbose(self, runner, temp_config_file):
        ...
        result = runner.invoke(cli, ["config", "validate", temp_config_file, "--verbose"])
    ...
    def test_config_validate_invalid(self, runner):
        ...
        try:
            result = runner.invoke(cli, ["config", "validate", temp_path])
    ...
    def test_config_show_tree(self, runner, temp_config_file):
        ...
        result = runner.invoke(cli, ["config", "show", temp_config_file, "--format", "tree"])
    ...
    def test_config_show_table(self, runner, temp_etl_config_file):
        ...
        result = runner.invoke(cli, ["config", "show", temp_etl_config_file, "--format", "table"])
    ...
    def test_config_show_graph(self, runner, temp_workflow_config_file):
        ...
        result = runner.invoke(
            cli, ["config", "show", temp_workflow_config_file, "--format", "graph"]
        )
...
class TestRunCommands:
    ...
    def test_run_execute_simple(self, runner, temp_config_file):
        ...
        result = runner.invoke(
            cli, ["run", "execute", temp_config_file, "--data", '{"input": "test"}']
        )
    ...
    def test_run_workflow_valid_path(self, runner, temp_workflow_config_file):
        ...
        result = runner.invoke(
            cli, ["run", "execute", temp_workflow_config_file, "--data", '{"is_valid": true}']
        )
    ...
    def test_run_workflow_invalid_path(self, runner, temp_workflow_config_file):
        ...
        result = runner.invoke(
            cli, ["run", "execute", temp_workflow_config_file, "--data", '{"is_valid": false}']
        )
...
class TestCLIErrorHandling:
    ...
    def test_invalid_json_data(self, runner, temp_config_file):
        ...
        result = runner.invoke(
            cli, ["run", "execute", temp_config_file, "--data", "invalid json {"]
        )
    ...
    def test_invalid_template_choice(self, runner):
        ...
        result = runner.invoke(cli, ["config", "create", "invalid_template"])
...
class TestCLIIntegration:
    ...
    def test_create_validate_execute_workflow(self, runner):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = runner.invoke(cli, ["config", "create", "basic", "--output", str(config_path)])
            assert result.exit_code == 0
            assert config_path.exists()

            # Step 2: Validate configuration
            result = runner.invoke(cli, ["config", "validate", str(config_path)])
            assert result.exit_code == 0
            assert "valid" in result.output.lower()

            # Step 3: Show configuration
            result = runner.invoke(cli, ["config", "show", str(config_path)])
            assert result.exit_code == 0
            assert "Basic_FSM" in result.output

            # Step 4: Execute FSM (if implemented)
            result = runner.invoke(
                cli, ["run", "execute", str(config_path), "--data", '{"test": "data"}']
            )
            # Check execution attempted
            assert result.exit_code == 0 or "not implemented" in result.output.lower()
    ...
    def test_etl_template_workflow(self, runner):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = runner.invoke(cli, ["config", "create", "etl", "--output", str(etl_path)])
            assert result.exit_code == 0

            # Validate ETL configuration
            result = runner.invoke(cli, ["config", "validate", str(etl_path), "--verbose"])
    ...
    def test_multiple_format_outputs(self, runner, temp_config_file):
        ...
        # Table format
        result = runner.invoke(cli, ["config", "show", temp_config_file, "--format", "table"])
        assert result.exit_code == 0
        assert "Name" in result.output

        # Graph format
        result = runner.invoke(cli, ["config", "show", temp_config_file, "--format", "graph"])
        assert result.exit_code == 0
        assert "mermaid" in result.output.lower()
...
class TestAdvancedCLIFeatures:
    ...
    def test_complex_transformation_config(self, runner):
            ...
            "arcs": [
                {
                    ...
                    "transform": {
                        "type": "inline",
                        "code": 'data["value"] = data.get("value", 0) * 2; data',
                    },
                },
        ...
        try:
            ...
            result = runner.invoke(cli, ["config", "validate", temp_path, "--verbose"])
    ...
    def test_resource_configuration(self, runner):
        ...
        try:
            result = runner.invoke(cli, ["config", "show", temp_path, "--format", "tree"])
...
class TestHistoryCLICommands:
    ...
    def test_list_with_entries_table(self, runner, tmp_path, history_dir, home_env):
        """Seeded history shows up in the table output with correct columns."""
        self._seed_history(
            history_dir,
            execution_id="exec-aaaaaaaaaa",
            fsm_name="etl_pipeline",
        )
        result = runner.invoke(cli, ["history", "list"], env=home_env)
    ...
    def test_list_filters_by_fsm_name(self, runner, tmp_path, history_dir, home_env):
        """``--fsm-name`` filters server-side via ``query_histories``."""
        self._seed_history(
            history_dir,
            execution_id="exec-keep-1",
            fsm_name="target_fsm",
        )
        self._seed_history(
            history_dir,
            execution_id="exec-skip-2",
            fsm_name="other_fsm",
        )

        result = runner.invoke(
            cli,
            ["history", "list", "--fsm-name", "target_fsm"],
            env=home_env,
        )
    ...
    def test_show_execution_found(self, runner, tmp_path, history_dir, home_env):
        """``show-execution <id>`` prints details for a known execution."""
        self._seed_history(
            history_dir,
            execution_id="exec-show-12",
            fsm_name="show_fsm",
        )
        result = runner.invoke(
            cli,
            ["history", "show-execution", "exec-show-12"],
            env=home_env,
        )
        assert result.exit_code == 0, result.output
    ...
    def test_show_execution_verbose_lists_steps(self, runner, tmp_path, history_dir, home_env):
        """``--verbose`` includes the per-step execution path."""
        self._seed_history(
            history_dir,
            execution_id="exec-verbose-1",
            fsm_name="verbose_fsm",
        )
        result = runner.invoke(
            cli,
    ...
    def test_show_execution_failed_run_reports_failure(
        ...
        result = runner.invoke(
            cli,
            ["history", "show-execution", "exec-failed-1"],
            env=home_env,
        )
        assert result.exit_code == 0, result.output
        assert "Failed steps: 1" in result.output

    def test_show_execution_not_found_exits_nonzero(self, runner, history_dir, home_env):
        """Unknown execution id exits with a non-zero status."""
        result = runner.invoke(
            cli,
            ["history", "show-execution", "does-not-exist"],
            env=home_env,
        )
        assert result.exit_code != 0
    ...
    def test_show_execution_in_progress_run_status_consistent(
        ...
        result = runner.invoke(
            cli,
            ["history", "show-execution", "exec-running-1"],
            env=home_env,
        )
        assert result.exit_code == 0, result.output
