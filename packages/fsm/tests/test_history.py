...
class TestExecutionHistory:
    ...
    def test_create_history(self):
        ...
        history = ExecutionHistory(
            fsm_name="test_fsm", execution_id="exec_123", data_mode=DataHandlingMode.COPY
        )
    ...
    def test_add_step(self):
        ...
        history = ExecutionHistory(fsm_name="test_fsm", execution_id="exec_123")

        # Add first step
        step1 = history.add_step(state_name="start", network_name="main", data={"value": 1})
    ...
    def test_update_step(self):
        ...
        # Update via history
        success = history.update_step(step.step_id, metrics={"processed": 10})
        assert success
    ...
    def test_step_failure(self):
        ...
        history.update_step(step.step_id, status=ExecutionStatus.FAILED, error=error)
    ...
    def test_resource_tracking(self):
        ...
        # Add resource usage
        step.add_resource_usage("database", {"queries": 5, "duration": 1.2})
        step.add_resource_usage("llm", {"tokens": 500, "duration": 3.5})
    ...
    def test_stream_tracking(self):
        ...
        step.update_stream_progress(chunks=10, records=1000, current_position=5000)
    ...
    def test_pruning(self):
        ...
        history = ExecutionHistory(
            fsm_name="test", execution_id="exec_123", data_mode=DataHandlingMode.DIRECT, max_depth=3
        )
...
class TestStorageBackends:
    ...
    async def test_memory_storage_basic(self):
        ...
        history = ExecutionHistory(fsm_name="test", execution_id="mem_123")
    ...
    async def test_file_storage(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Save history
            history = ExecutionHistory(fsm_name="test", execution_id="file_123")
            history.add_step("start", "main")
    ...
    async def test_step_storage(self):
        ...
        # Create step
        step = ExecutionStep(
            step_id="step_1", state_name="process", network_name="main", timestamp=time.time()
        )
        ...
        # Filter steps
        steps = await storage.load_steps("exec_123", filters={"state_name": "process"})
        assert len(steps) == 1

        steps = await storage.load_steps("exec_123", filters={"state_name": "other"})
        assert len(steps) == 0
    ...
    async def test_statistics(self):
        ...
        for i in range(3):
            ...
            if i == 1:
                ...
                history.update_step(
                    step.step_id, status=ExecutionStatus.FAILED, error=Exception("Error")
                )
            history.finalize()
    ...
    async def test_mode_specific_config(self):
        ...
        for mode in [DataHandlingMode.COPY, DataHandlingMode.REFERENCE, DataHandlingMode.DIRECT]:
            history = ExecutionHistory(
                fsm_name="test", execution_id=f"{mode.value}_123", data_mode=mode
            )
            history.add_step("start", "main")
            history.finalize()
    ...
    async def test_concurrent_access(self):
        ...
        async def save_history(i: int):
            history = ExecutionHistory(fsm_name="concurrent", execution_id=f"concurrent_{i}")
...
class TestStorageFactory:
    ...
    def test_factory_create(self):
        ...
        with tempfile.NamedTemporaryFile(suffix=".json", delete=True) as tmp:
            config = StorageConfig(
                backend=StorageBackend.FILE, connection_params={"path": tmp.name}
            )
            storage = StorageFactory.create(config)
