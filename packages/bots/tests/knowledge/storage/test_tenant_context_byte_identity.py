...
def test_file_snapshot_paths_none_match_pre_adoption(tmp_path: Path) -> None:
    ...
    assert b._snapshot_file("my_kb", "abc123") == tmp_path / "my_kb" / "_snapshots" / "abc123.json"
...
def test_file_content_paths_never_tenant_scoped(tmp_path: Path) -> None:
    ...
    assert b._file_path("my_kb", "intro.md") == tmp_path / "my_kb" / "content" / "intro.md"
...
def test_s3_metadata_key_none_matches_pre_adoption() -> None:
    ...
    assert b._metadata_key("my_kb", ctx=SingleTenantContext("my_kb")) == "kbs/my_kb/_metadata.json"
