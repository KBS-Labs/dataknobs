    ...
    flow = ConversationFlow(
        ...
        states={
            ...
            "greeting": FlowState(
                ...
                transitions={"need_help": "collect_issue", "just_browsing": "end_browsing"},
                transition_conditions={
                    "need_help": keyword_condition(
                        ["help", "issue", "problem", "support"], case_sensitive=False
                    ),
                    "just_browsing": keyword_condition(
                        ["browse", "look", "explore", "no thanks"], case_sensitive=False
                    ),
                },
            ),
            ...
            "collect_issue": FlowState(
                ...
                transition_conditions={
                    "technical": keyword_condition(
                        ["technical", "bug", "error", "broken", "crash"], case_sensitive=False
                    ),
                    "billing": keyword_condition(
                        ["billing", "payment", "charge", "invoice", "cost"], case_sensitive=False
                    ),
                    "account": keyword_condition(
                        ["account", "login", "password", "access"], case_sensitive=False
                    ),
            ...
            "tech_support": FlowState(
                ...
                transitions={"resolved": "end_resolved", "escalate": "end_escalate"},
                transition_conditions={
                    "resolved": keyword_condition(
                        ["yes", "solved", "fixed", "worked", "thanks"], case_sensitive=False
                    ),
                    "escalate": keyword_condition(
                        ["no", "still", "didn't work", "escalate", "manager"], case_sensitive=False
                    ),
                },
            ),
            ...
            "billing_support": FlowState(
                ...
                transition_conditions={
                    "resolved": keyword_condition(
                        ["yes", "solved", "understood", "thanks"], case_sensitive=False
                    ),
                    "escalate": keyword_condition(
                        ["no", "still", "dispute", "escalate", "manager"], case_sensitive=False
                    ),
                },
            ),
            ...
            "account_support": FlowState(
                ...
                transition_conditions={
                    "resolved": keyword_condition(
                        ["yes", "solved", "logged in", "thanks"], case_sensitive=False
                    ),
                    "escalate": keyword_condition(
                        ["no", "still", "can't access", "escalate"], case_sensitive=False
                    ),
                },
            ),
            ...
            "clarify_issue": FlowState(
                ...
                transitions={"retry": "collect_issue", "give_up": "end_unclear"},
                transition_conditions={
                    "retry": keyword_condition(
                        ["let me explain", "try again", "here's my issue"], case_sensitive=False
                    ),
            ...
            "end_browsing": FlowState(
                prompt_name="goodbye_browsing", transitions={}, transition_conditions={}
            ),
            "end_resolved": FlowState(
                prompt_name="goodbye_resolved", transitions={}, transition_conditions={}
            ),
            "end_escalate": FlowState(
                prompt_name="escalation_message", transitions={}, transition_conditions={}
            ),
            "end_unclear": FlowState(
                prompt_name="goodbye_unclear", transitions={}, transition_conditions={}
            ),
        },
    )
    ...
    flow = ConversationFlow(
        ...
        states={
            "introduce": FlowState(
                ...
                transitions={"interested": "qualify_needs", "not_interested": "end_no_interest"},
                transition_conditions={
                    "interested": keyword_condition(
                        ["interested", "tell me more", "yes", "sounds good"], case_sensitive=False
                    ),
                    "not_interested": keyword_condition(
                        ["not interested", "no thanks", "busy", "not now"], case_sensitive=False
                    ),
                },
            ),
            "qualify_needs": FlowState(
                prompt_name="qualify_questions",
                transitions={"good_fit": "present_solution", "poor_fit": "end_poor_fit"},
                transition_conditions={
                    "good_fit": keyword_condition(
                        ["yes", "need", "looking for", "help with"], case_sensitive=False
                    ),
                    "poor_fit": keyword_condition(
                        ["no", "don't need", "already have"], case_sensitive=False
                    ),
                },
            ),
            "present_solution": FlowState(
                ...
                transition_conditions={
                    "discuss_price": keyword_condition(
                        ["price", "cost", "how much", "pricing"], case_sensitive=False
                    ),
                    "not_convinced": keyword_condition(
                        ["but", "however", "concern", "not sure"], case_sensitive=False
                    ),
                },
            ),
            "handle_objection": FlowState(
                ...
                transition_conditions={
                    "convinced": keyword_condition(
                        ["makes sense", "okay", "alright", "fair"], case_sensitive=False
                    ),
            ...
            "pricing_discussion": FlowState(
                ...
                transitions={"close": "end_close", "think_about_it": "end_think"},
                transition_conditions={
                    "close": keyword_condition(
                        ["sounds good", "let's do it", "sign me up", "yes"], case_sensitive=False
                    ),
                    "think_about_it": keyword_condition(
                        ["think about it", "get back", "consider", "maybe"], case_sensitive=False
                    ),
                },
            ),
            # End states
            "end_no_interest": FlowState(
                prompt_name="goodbye_no_interest", transitions={}, transition_conditions={}
            ),
            "end_poor_fit": FlowState(
                prompt_name="goodbye_poor_fit", transitions={}, transition_conditions={}
            ),
            "end_follow_up": FlowState(
                prompt_name="schedule_follow_up", transitions={}, transition_conditions={}
            ),
            "end_think": FlowState(
                prompt_name="goodbye_think", transitions={}, transition_conditions={}
            ),
            "end_close": FlowState(
                prompt_name="congratulations_close", transitions={}, transition_conditions={}
            ),
        },
    )
...
async def example_usage():
    ...
    for state_name, state in support_flow.states.items():
        transitions_str = ", ".join(
            f"{cond} -> {target}" for cond, target in state.transitions.items()
        )
