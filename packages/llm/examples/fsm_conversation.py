...
class IntentAnalyzer:
    ...
    async def analyze(input_text: str, context: ConversationContext) -> Dict[str, Any]:
        ...
        if any(word in input_lower for word in ["?", "what", "how", "why", "when", "where", "who"]):
            ...
            word in input_lower for word in ["do", "execute", "run", "create", "delete", "update"]
        ):
...
Be concise and helpful.""")
    ...
    def _prepare_template_params(
        ...
        if intent == "question":
            ...
            params["response"] = (
                f"Based on my analysis, here's information about {params['topic']}..."
            )

            # Add context if available
            if context.history:
                params["context"] = (
                    f"our previous discussion about {context.current_topic or 'this topic'}"
                )
...
class LLMConversationFSM:
    ...
    def _create_fsm(self) -> SimpleFSM:
        ...
        custom_functions = {
            ...
            "check_analysis_success": lambda data, context=None: (
                data.get("analysis", {}).get("confidence", 0) >= 0.3
            ),
            "check_analysis_failure": lambda data, context=None: (
                data.get("analysis", {}).get("confidence", 1) < 0.3
            ),
            "check_generation_success": lambda data, context=None: (
                "response" in data and not data.get("error")
            ),
        ...
        fsm = SimpleFSM(
            config, data_mode=DataHandlingMode.REFERENCE, custom_functions=custom_functions
        )
        return fsm
    ...
    async def _analyze_input(self, data: Dict[str, Any], context=None) -> Dict[str, Any]:
        ...
        logger.info(
            f"Analyzed input - Intent: {analysis['intent']}, Confidence: {analysis['confidence']}"
        )
        return actual_data
    ...
    async def _generate_response(self, data: Dict[str, Any], context=None) -> Dict[str, Any]:
        ...
        try:
            response = await self.generator.generate(
                actual_data["intent"], actual_data["input"], actual_data["analysis"], self.context
            )
    ...
    async def _update_context(self, data: Dict[str, Any], context=None) -> Dict[str, Any]:
        ...
        # Add to history
        self.context.history.append({"role": "user", "content": actual_data["input"]})
        self.context.history.append({"role": "assistant", "content": actual_data["response"]})
...
def get_llm_config_from_env() -> LLMConfig | None:
    ...
    if provider == "ollama":
        ...
        return LLMConfig(
            provider="ollama", model=model, temperature=temperature, max_tokens=max_tokens
        )
