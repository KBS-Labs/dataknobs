...
class LLMWorkflow:
    ...
    async def _execute_step(self, step: LLMStep, input_data: Dict[str, Any]) -> Any:
        ...
        while retry_count <= step.max_retries:
            try:
                ...
                if self.config.track_tokens and response.usage:
                    self._context["total_tokens"] = self._context.get(
                        "total_tokens", 0
...
def create_simple_llm_workflow(
    ...
    config = LLMWorkflowConfig(
        ...
        default_model_config=LLMConfig(provider=provider, model=model, **kwargs),
    )
...
def create_rag_workflow(
    ...
    **kwargs,
    ...
    config = LLMWorkflowConfig(
        ...
        steps=[],
        default_model_config=LLMConfig(provider=provider, model=model, **kwargs),
        rag_config=RAGConfig(retriever_type=retriever_type, top_k=top_k),
    )
