...
from dataknobs_config import Config, InvalidReferenceError, ConfigNotFoundError
...
class TestReferenceResolution:
    ...
    def test_resolve_by_index(self):
        ...
        config = Config({
            "cache": [
                {"name": "redis1", "port": 6379},
                {"name": "redis2", "port": 6380}
            ]
        })
    ...
    def test_resolve_no_selector(self):
        ...
        config = Config({
            "database": [{"name": "only", "host": "localhost"}]
        })
