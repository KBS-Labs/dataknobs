...
def test_drop_non_embedded_symbols_fn():
    assert dk_norm.drop_non_embedded_symbols_fn(
        '   .te-st. .this. .   .#$now..   '
    ) == '   te-st this    now   '


def test_drop_embedded_symbols_fn():
    assert dk_norm.drop_embedded_symbols_fn(
        '   .te-st. .this. .   .#$now..   '
    ) == '   .test. .this. .   .#$now..   '
...
def test_get_hyphen_slash_expansions_fn():
    ...
    assert f(
        'abc-def', subs='/: ', add_self=True, do_split=True
    ) == {
        'abc def', 'abc-def', 'def', 'abc', 'abc/def', 'abc:def'
    }

    assert f(
        'abc-def', subs='/: ', add_self=True, do_split=False
    ) == {
        'abc def', 'abc-def', 'abc/def', 'abc:def'
    }

    assert f(
        'abc-def', subs=[' ', ''], add_self=False, do_split=True
    ) == {
        'abc def', 'abc', 'def', 'abcdef'
    }

    assert f(
        'this is a test', subs='-', do_split=True, min_split_token_len=2
    ) == {
        'this is a test', 'this-is-a-test',
    }

    assert f(
        'a/b', subs=[' or '], add_self=True, do_split=True,
...
def get_year_variations_fn():
    assert dk_norm.year_variations_fn(2003) == {
        'twenty 03', 'twenty oh three', '2003', 'two thousand, three',
        'twenty and three', 'two thousand and three', 'oh three',
        'two thousand oh three', 'three', '03', 'two thousand 03',
    }

    assert dk_norm.year_variations_fn(2023) == {
        '23', '2023', 'two thousand, twenty-three', 'twenty-three',
        'two thousand and twenty-three', 'two thousand twenty-three',
        'two thousand 23', 'twenty 23', 'twenty and twenty-three',
        'twenty twenty-three',
    }

    assert dk_norm.year_variations_fn(1973) == {
        'nineteen and seventy-three', 'seventy-three',
        'nineteen hundred seventy-three',
        'nineteen hundred and seventy-three', '1973',
