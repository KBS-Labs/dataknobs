...
async def test_async_cancelled_error_always_propagates() -> None:
    class CancellingClosable:
        async def close(self) -> None:
            raise asyncio.CancelledError()

    captured: list[BaseException] = []
    # CancelledError is a BaseException — never swallowed even with on_error.
    with pytest.raises(asyncio.CancelledError):
        await close_if_owned(
            CancellingClosable(), True, on_error=captured.append
        )
    assert captured == []
