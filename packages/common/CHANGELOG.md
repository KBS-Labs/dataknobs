...
- `SyncLoopBridge` and the `run_coro_sync` convenience function in
  ...
  bar rejects) is avoided by construction. This is the async→sync
  counterpart to the existing sync→async `aiter_sync_in_thread`. Use a
  long-lived `SyncLoopBridge` for a synchronous wrapper that makes repeated
  calls; use `run_coro_sync` for one-off calls.
