...
class InMemoryEventBus(StructuredConfigConsumer[MemoryEventBusConfig]):
    ...
    a typed :class:`MemoryEventBusConfig`, a dict via ``config=``, or
    loose ``**kwargs``. Today ``MemoryEventBusConfig`` has no fields so
    all three shapes degenerate to ``InMemoryEventBus()``.
    ...
    CONFIG_CLS: ClassVar[type[MemoryEventBusConfig]] = MemoryEventBusConfig
