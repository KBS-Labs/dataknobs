...
A record holds :class:`~dataknobs_common.fields.Field` values and knows
nothing about which subclasses of ``Field`` exist -- ``Field.copy`` and
``Field.from_dict`` are what keep a subclass its own type through a copy or a
round trip.
"""
...
class Record:
    ...
    Example:
        ```python
        from dataknobs_common import Record, Field, FieldType
    ...
    def copy(self, deep: bool = True) -> Record:
        ...
        if deep:
            # Field.copy() is polymorphic, so a subclass comes back as itself
            # and this loop never has to know which subclasses exist.
            new_fields = OrderedDict((name, field.copy()) for name, field in self.fields.items())
