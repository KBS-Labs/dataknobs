...
class RecordFetcher(ABC):
    ...
    def __init__(self, id_field_name="id", fields_to_retrieve=None, one_based_ids=False):
    ...
    @abstractmethod
    def get_records(
        self, ids: List[int], one_based: bool = False, fields_to_retrieve: List[str] = None
    ) -> pd.DataFrame:
...
class DotenvPostgresConnector:
    def __init__(self, host=None, db=None, user=None, pwd=None, pvname: str = ".project_vars"):
        ...
        self.host = (
            os.getenv("POSTGRES_HOST", config.get("POSTGRES_HOST", "localhost"))
            if host is None
            else host
        )
        self.database = (
            os.getenv("POSTGRES_DB", config.get("POSTGRES_DB", "postgres")) if db is None else db
        )
        self.user = (
            os.getenv("POSTGRES_USER", config.get("POSTGRES_USER", "postgres"))
            if user is None
            else user
        )
        self.password = (
            os.getenv("POSTGRES_PASSWORD", config.get("POSTGRES_PASSWORD", None))
            if pwd is None
            else pwd
        )
...
class PostgresRecordFetcher(RecordFetcher):
    ...
    def __init__(
        self, db, table_name, id_field_name="id", fields_to_retrieve=None, one_based_ids=False
    ):
...
class DataFrameRecordFetcher(RecordFetcher):
    ...
    def __init__(
        self, df: pd.DataFrame, id_field_name="id", fields_to_retrieve=None, one_based_ids=False
    ):
    ...
    def get_records(
        ...
        if one_based != self.one_based:
            offset = 1 if self.one_based else -1
        ids = [an_id + offset for an_id in ids]
