...
def test_get_value_by_key():
    assert llm_utils.get_value_by_key(
        {"foo": {"bar": "baz"}},
        "foo.bar",
        "Undefined"
    ) == "baz"
...
def test_prompt_tree_serialization_roundtrip():
    ...
    assert data == data2
    assert (
        re_pt.message.to_json(with_metadata=True) ==
        pt2_1_1.message.to_json(with_metadata=True)
    )
    ...
    assert (
        re_pt2.message.to_json(with_metadata=True) ==
        re_pt.message.to_json(with_metadata=True)
    )
...
def test_get_duration_and_apply():
    ...
    root_pt = llm_utils.PromptTree(role="1", content="1", metadata={"id": 1})
    pt1 = root_pt.add_message(role="2", content="2", metadata={
        "id": 3,
        "execution_data": {
            "starttime": tm30,
            "endtime": tm20,
    ...
    pt2 = root_pt.add_message(role="3", content="3", metadata={
        "id": 3,
        "execution_data": {
            "starttime": tm20,
            "endtime": tm10,
    ...
    pt1_1 = pt1.add_message(role="4", content="4", metadata={"id": 4})
    pt1_2 = pt1.add_message(role="5", content="5", metadata={"id": 5})
    pt2_1 = pt2.add_message(role="6", content="6", metadata={
        "id": 3,
        "execution_data": {
            "starttime": tm10,
            "endtime": tm0,
