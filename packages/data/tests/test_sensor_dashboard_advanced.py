...
class TestSensorDashboardRangeOperators:
    ...
    def test_anomaly_detection_with_not_between(self, dashboard_with_data):
        ...
        temp_anomaly = any(r.temperature < 15 or r.temperature > 30 for r in anomalies)
        assert temp_anomaly

        # Verify at least one humidity anomaly
        humidity_anomaly = any(r.humidity < 30 or r.humidity > 70 for r in anomalies)
...
class TestSensorDashboardBooleanLogic:
    ...
    def dashboard_with_critical_data(self):
        ...
        readings = [
            ...
            SensorReading("critical_1", now - timedelta(hours=1), 25.0, 50.0, 10.0, "server_room"),
            # Critical: Extreme temperature
            SensorReading("critical_2", now - timedelta(hours=1), 42.0, 60.0, 80.0, "warehouse"),
            # Critical: Very cold
            SensorReading("critical_2", now - timedelta(hours=2), 3.0, 40.0, 75.0, "warehouse"),
            # Normal reading
            SensorReading("normal_1", now - timedelta(hours=1), 22.0, 45.0, 90.0, "office"),
            # Old reading for maintenance check
            SensorReading("critical_1", now - timedelta(days=8), 24.0, 48.0, 25.0, "server_room"),
        ]
    ...
    def test_or_operator_multiple_locations(self, dashboard_with_critical_data):
        ...
        readings = dashboard.get_sensors_by_multiple_locations(["server_room", "warehouse"])
    ...
    def test_complex_critical_conditions(self, dashboard_with_critical_data):
        ...
        critical = dashboard.get_critical_sensors(
            battery_threshold=20.0, locations=["server_room", "warehouse"]
        )
        ...
        has_extreme_temp = any(r.temperature < 5 or r.temperature > 40 for r in critical)
    ...
    def test_not_operator_maintenance(self, dashboard_with_critical_data):
        ...
        old_reading = any((datetime.now() - r.timestamp).days >= 7 for r in maintenance)
        assert old_reading
    ...
    def test_combined_and_or_logic(self, dashboard_with_critical_data):
        ...
        critical_query = (
            Query()
            .filter("metadata.type", Operator.EQ, "sensor_reading")
            .or_(Filter("battery", Operator.LT, 50), Filter("temperature", Operator.GT, 30))
        )
...
class TestQueryBuilderIntegration:
    ...
    def test_query_builder_step_by_step(self, populated_dashboard):
        ...
        temp_humidity = QueryBuilder().or_(
            Filter("temperature", Operator.GT, 25), Filter("humidity", Operator.LT, 40)
        )
