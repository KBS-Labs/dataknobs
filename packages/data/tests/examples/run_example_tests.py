...
class ExampleTestRunner:
    ...
    def run_all_tests(self, filter_pattern: str = None) -> bool:
        ...
        for test_file in test_files:
            ...
            self.results.append({"test": test_file, "passed": success, "time": elapsed})
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Run tests for DataKnobs vector store examples")
    parser.add_argument("--verbose", "-v", action="store_true", help="Enable verbose output")
    parser.add_argument("--filter", "-f", type=str, help="Filter tests by pattern")
    parser.add_argument(
        "--validate-only", action="store_true", help="Only validate examples without running tests"
    )
