...
class TestBasicVectorSearchIntegration:
    ...
    async def test_similarity_metrics(self, vector_db):
        ...
        doc1 = Record(
            {
                ...
                "embedding": VectorField(
                    TestEmbedding.generate("machine learning AI"), dimensions=384
                ),
            }
        )
        doc2 = Record(
            {
                ...
                "embedding": VectorField(
                    TestEmbedding.generate("web javascript html"), dimensions=384
                ),
            }
        )
        ...
        results = await vector_db.vector_search(
            query_vector=ml_query, k=2, vector_field="embedding"
        )
...
class TestTextToVectorSyncIntegration:
    ...
    async def test_synchronizer_workflow(self, vector_db):
        ...
        search_results = await vector_db.vector_search(
            query_vector=query_embedding, k=2, vector_field="embedding"
        )
    ...
    async def test_change_tracking(self, vector_db):
        ...
        record1 = await vector_db.create(
            Record(
                {
                    "title": "Original",
                    "content": "Original content",
                    "embedding": VectorField(
                        TestEmbedding.generate("original content"), dimensions=384
                    ),
                }
            )
        )
        ...
        await tracker.start_tracking(tracked_fields=["title", "content"], vector_field="embedding")
...
class TestMigrationIntegration:
    ...
    async def test_database_migration(self):
        ...
        try:
            ...
            search_results = await vector_db.vector_search(
                query_vector=query, k=2, vector_field="embedding"
            )
    ...
    async def test_incremental_vectorization(self, vector_db):
        ...
        docs = [
            {"title": f"Document {i}", "content": f"Content for document {i}"} for i in range(5)
        ]
...
class TestHybridSearchIntegration:
    ...
    async def test_hybrid_search_methods(self, vector_db):
        ...
        for doc in docs:
            ...
            record = Record({**doc, "embedding": VectorField(embedding, dimensions=384)})
        ...
        has_ml_or_ai = any(
            ...
            or "Python machine learning" in title.lower()
    ...
    async def test_reciprocal_rank_fusion(self, vector_db):
        ...
        # Add test documents
        docs = [{"id": i, "title": f"Doc {i}", "content": f"Content {i}"} for i in range(5)]
...
class TestPerformanceIntegration:
    ...
    async def test_large_embedding_dimensions(self):
        ...
        for dim in dimensions:
            ...
            factory = AsyncDatabaseFactory()
            db = factory.create(backend="memory", vector_enabled=True, vector_metric="cosine")
            await db.connect()
            ...
            try:
                ...
                record = Record(
                    {"title": f"Dim {dim}", "embedding": VectorField(embedding, dimensions=dim)}
                )
                ...
                results = await db.vector_search(query_vector=query, k=1, vector_field="embedding")
...
async def test_end_to_end_workflow():
    ...
    try:
        ...
        search_results = await db.vector_search(query_vector=query, k=2, vector_field="embedding")
        ...
        new_results = await db.vector_search(query_vector=query, k=2, vector_field="embedding")
