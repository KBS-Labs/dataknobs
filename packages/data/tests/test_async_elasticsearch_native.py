...
async def mock_es_client():
    ...
    client.get = AsyncMock(
        return_value={"_id": "test-id", "_source": {"data": {"name": "test"}, "metadata": {}}}
    )
    ...
    client.search = AsyncMock(
        return_value={
            "hits": {"hits": [{"_id": "1", "_source": {"data": {"name": "doc1"}, "metadata": {}}}]}
        }
    )

    # Mock scroll operations
    client.scroll = AsyncMock(return_value={"_scroll_id": "scroll-123", "hits": {"hits": []}})
...
async def es_db():
    ...
    config = {"hosts": ["localhost:9200"], "index": "test_index"}
...
async def test_read_record(es_db, mock_es_client):
    ...
    mock_es_client.get.assert_called_once_with(index="test_index", id="test-id")
...
async def test_delete_record(es_db, mock_es_client):
    ...
    assert result is True
    mock_es_client.delete.assert_called_once_with(index="test_index", id="test-id", refresh=True)
...
async def test_exists_record(es_db, mock_es_client):
    ...
    assert result is True
    mock_es_client.exists.assert_called_once_with(index="test_index", id="test-id")
...
async def test_stream_read(es_db, mock_es_client):
    ...
    mock_es_client.search.return_value = {
        ...
        "hits": {
            "hits": [
                {"_id": "1", "_source": {"data": {"name": "doc1"}, "metadata": {}}},
                {"_id": "2", "_source": {"data": {"name": "doc2"}, "metadata": {}}},
            ]
        },
    }
...
def test_extract_bulk_index_ids_upsert_drops_failed_items():
    ...
    got = AsyncElasticsearchDatabase._extract_bulk_index_ids(response, ["a", "b", "c"])
...
def test_extract_bulk_index_ids_server_id_path_drops_failed():
    ...
    response = {
        "items": [
            ...
            {"index": {"_id": "s2", "status": 400, "error": {"type": "mapper_parsing_exception"}}},
...
async def test_upsert_batch_drops_failed_item(es_db, mock_es_client):
    ...
    ids = await es_db.upsert_batch([Record({"v": 1}, id="a"), Record({"v": 2}, id="b")])
...
async def test_create_batch_drops_failed_item(es_db, mock_es_client):
    ...
    ids = await es_db.create_batch([Record({"v": 1}, id="a"), Record({"v": 2}, id="b")])
...
async def test_create_batch_conflict_raises(es_db, mock_es_client):
    ...
    with pytest.raises(DuplicateRecordError):
        await es_db.create_batch([Record({"v": 1}, id="a"), Record({"v": 2}, id="b")])
