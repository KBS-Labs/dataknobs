...
class TestFactoryVectorIntegration:
    ...
    def test_sqlite_vector_enabled(self, factory):
        ...
        with tempfile.NamedTemporaryFile(suffix=".db", delete=True) as f:
            ...
            try:
                ...
                vector_field = VectorField(name="embedding", value=vec, dimensions=3)

                record = Record(
                    data=OrderedDict({"embedding": vector_field}), metadata={"test": "factory"}
                )
    ...
    def test_vector_enabled_all_backends(self, factory):
        ...
        with tempfile.NamedTemporaryFile(suffix=".json", delete=False) as tmp:
            db2 = factory.create(backend="file", path=tmp.name, vector_enabled=True)
    ...
    def test_sqlite_without_vector_enabled(self, factory):
        ...
        with tempfile.NamedTemporaryFile(suffix=".db", delete=True) as f:
            db = factory.create(backend="sqlite", path=f.name, table="test_normal")
    ...
    def test_postgres_vector_enabled(self, factory):
        ...
        try:
            ...
            record = Record(
                data=OrderedDict({"embedding": vector_field}), metadata={"test": "postgres_factory"}
            )
    ...
    def test_elasticsearch_vector_enabled(self, factory):
        ...
        try:
            ...
            record = Record(
                data=OrderedDict({"embedding": vector_field}), metadata={"test": "es_factory"}
            )
            ...
            try:
                ...
                es = Elasticsearch([{"host": elasticsearch_host, "port": port, "scheme": "http"}])
