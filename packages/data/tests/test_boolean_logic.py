...
class TestBooleanLogicQueries:
    ...
    def test_simple_or_query(self, db_with_data):
        ...
        query = Query().or_(
            Filter("city", Operator.EQ, "New York"), Filter("city", Operator.EQ, "Los Angeles")
        )
    ...
    def test_not_query(self, db_with_data):
        ...
        query = (
            ...
            .not_(Filter("city", Operator.EQ, "New York"))
        )
    ...
    def test_between_with_or(self, db_with_data):
        ...
        query = Query().or_(
            Filter("age", Operator.BETWEEN, (25, 30)), Filter("city", Operator.EQ, "Chicago")
        )
    ...
    def test_complex_query_serialization(self):
        ...
        builder.or_(Filter("priority", Operator.GT, 5), Filter("urgent", Operator.EQ, True))
...
class TestAsyncBooleanLogic:
    ...
    async def test_async_or_query(self, async_db_with_data):
        ...
        query = Query().or_(
            Filter("type", Operator.EQ, "sensor"), Filter("location", Operator.EQ, "room1")
        )
    ...
    async def test_async_complex_conditions(self, async_db_with_data):
        ...
        query = Query().or_(sensor_high, Filter("location", Operator.EQ, "room1"))
...
class TestLogicConditions:
    ...
    def test_nested_logic_conditions(self):
        ...
        and_condition = LogicCondition(
            operator=LogicOperator.AND,
            conditions=[or_condition, FilterCondition(Filter("active", Operator.EQ, True))],
        )
