...
class TestSQLiteVectorSupport:
    ...
    def test_create_record_with_vector(self, db):
        ...
        vector_field = VectorField(name="embedding", value=vector, dimensions=4)

        # Create a record
        record = Record(data=OrderedDict({"embedding": vector_field}), metadata={"test": "value"})
    ...
    def test_vector_search(self, db):
        ...
        ids = db.add_vectors(
            vectors=vectors, metadata=[{"index": i} for i in range(5)], field_name="embedding"
        )
    ...
    def test_vector_search_with_filter(self, db):
        ...
        for i, (vec, cat) in enumerate(zip(vectors, categories, strict=True)):
            ...
            record = Record(
                data=OrderedDict({"embedding": vector_field, "category": cat}),
                metadata={"index": i},
            )
    ...
    def test_different_distance_metrics(self, db):
        ...
        results_euclidean = db.vector_search(
            query_vector=vec1, k=2, metric=DistanceMetric.EUCLIDEAN
        )
        assert results_euclidean[0].record.metadata["name"] == "vec1"  # vec1 should be first

        # Test dot product
        results_dot = db.vector_search(query_vector=vec1, k=2, metric=DistanceMetric.DOT_PRODUCT)
    ...
    def test_update_record_with_vector(self, db):
        ...
        record = Record(data=OrderedDict({"embedding": vector_field}), metadata={"version": 1})
        record_id = db.create(record)
        ...
        updated_record = Record(
            data=OrderedDict({"embedding": updated_field}), metadata={"version": 2}
        )
        ...
        # Verify update
        retrieved = db.read(record_id)
        np.testing.assert_array_almost_equal(retrieved.fields["embedding"].value, vec2)
    ...
    def test_batch_operations_with_vectors(self, db):
        ...
        for i in range(10):
            ...
            record = Record(
                data=OrderedDict({"embedding": vector_field}), metadata={"batch_index": i}
            )
    ...
    def test_empty_vector_handling(self, db):
        ...
        record = Record(
            data=OrderedDict({"text": Field(name="text", value="Some text without a vector")}),
        ...
        record_with_vector = Record(
            data=OrderedDict({"embedding": vector_field}), metadata={"has_vector": True}
        )
