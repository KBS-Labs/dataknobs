...
class TestMigratorAdvanced:
    ...
    def test_migrate_with_skip_logic(self):
        ...
        progress = migrator.migrate(
            source=source, target=target, transform=FilteringTransformer(), batch_size=3
        )
    ...
    def test_migrate_stream_basic(self):
        ...
        migrator = Migrator()
        progress = migrator.migrate_stream(source=source, target=target, config=config)
    ...
    def test_migrate_stream_with_transform(self):
        ...
        progress = migrator.migrate_stream(
            source=source, target=target, transform=transformer, config=config
        )
    ...
    def test_migrate_stream_with_filter(self):
        ...
        # Add mixed records
        for i in range(30):
            record = Record({"id": i, "type": "important" if i % 3 == 0 else "regular", "value": i})
            source.create(record)
        ...
        migrator = Migrator()
        progress = migrator.migrate_stream(source=source, target=target, query=query)
    ...
    def test_migrate_stream_with_errors(self):
        ...
        migrator = Migrator()
        progress = migrator.migrate_stream(
            source=source, target=target, transform=FailingTransformer(), config=config
        )
    ...
    def test_migrate_stream_with_migration(self):
        ...
        # Add test records
        for i in range(25):
            record = Record({"user_id": i, "username": f"user_{i}", "old_field": "deprecated"})
            source.create(record)
        ...
        migrator = Migrator()
        progress = migrator.migrate_stream(source=source, target=target, transform=migration)
    ...
    def test_migrate_parallel(self):
        ...
        for partition in range(4):
            for i in range(25):
                record = Record({"id": f"p{partition}_r{i}", "partition_id": partition, "value": i})
                source.create(record)

        migrator = Migrator()
        progress = migrator.migrate_parallel(
            source=source, target=target, partitions=4, partition_field="partition_id"
        )
    ...
    def test_migrate_parallel_with_transform(self):
        ...
        for partition in range(2):
            for i in range(10):
                record = Record({"partition_id": partition, "old_value": i})
                source.create(record)
        ...
        migrator = Migrator()
        progress = migrator.migrate_parallel(
            source=source, target=target, transform=transformer, partitions=2
        )
    ...
    async def test_migrate_async_basic(self):
        ...
        migrator = Migrator()
        progress = await migrator.migrate_async(source=source, target=target)
    ...
    async def test_migrate_async_with_transform(self):
        ...
        # Add test records
        for i in range(30):
            record = Record({"original_id": i, "value": i * 5})
            await source.create(record)
    ...
    async def test_migrate_async_with_errors(self):
        ...
        migrator = Migrator()
        progress = await migrator.migrate_async(
            source=source, target=target, transform=AsyncFailingTransformer(), config=config
        )
    ...
    def test_validate_migration_with_sample(self):
        ...
        is_valid, issues = migrator.validate_migration(source, target, sample_size=10)
        assert is_valid
        assert len(issues) == 0
        ...
        is_valid, issues = migrator.validate_migration(source, target2, sample_size=50)
    ...
    def test_migrate_with_progress_callback(self):
        ...
        final_progress = migrator.migrate(
            source=source, target=target, batch_size=10, on_progress=track_progress
        )
    ...
    def test_migrate_stream_without_count(self):
        ...
        migrator = Migrator()
        progress = migrator.migrate_stream(source=source, target=target)
...
class TestMigratorConflictPolicy:
    ...
    async def test_async_skip(self):
        ...
        await _aseed(src, [1, 2, 3], "src")
        await _aseed(tgt, [2], "old")
        progress = await Migrator().migrate_async(src, tgt, config=StreamConfig(on_conflict="skip"))
        assert progress.failed == 0
        assert progress.skipped >= 1
    ...
    def test_streaming_skip_accounting(self):
        ...
        _seed(src, [1, 2, 3], "src")
        _seed(tgt, [2], "old")
        progress = Migrator().migrate_stream(src, tgt, config=StreamConfig(on_conflict="skip"))
        assert progress.skipped == 1
...
class TestMigrateStreamProcessedAccounting:
    ...
    async def test_migrate_async_counts_filtered_record_once(self):
        ...
        progress = await Migrator().migrate_async(src, tgt, transform=_FilterEven())
...
class TestMigratorBatchedWriteVerbs:
    ...
    def test_batch_failure_falls_back_to_per_record_attribution(self):
        ...
        seen: list[str] = []
        progress = Migrator().migrate(src, tgt, on_error=lambda _e, r: seen.append(r.id) or True)
...
class TestMigratorBatchShortfall:
    ...
    def test_shortfall_handler_veto_raises(self):
        ...
        progress = MigrationProgress().start()
        with pytest.raises(OperationError, match="confirmed 2 of 3 records"):
            Migrator()._write_batch(tgt, batch, progress, on_error=lambda _e, _r: False)
