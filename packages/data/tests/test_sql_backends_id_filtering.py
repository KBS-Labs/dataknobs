...
class TestSqlBackendsIdFiltering:
    ...
    def test_id_field_in_filter(self, db):
        ...
        # Test NOT_IN
        query = (
            Query().filter("id", Operator.NOT_IN, ["rec_0", "rec_2", "rec_4"]).sort_by("id", "asc")
        )
        results = db.search(query)
        assert len(results) == 2
    ...
    def test_id_field_between_filter(self, db):
        ...
        # Test NOT_BETWEEN
        query = (
            Query().filter("id", Operator.NOT_BETWEEN, ["rec_001", "rec_003"]).sort_by("id", "asc")
        )
        results = db.search(query)
        assert len(results) == 2
    ...
    def test_id_field_combined_with_data_filters(self, db):
        ...
        query = (
            Query()
            .filter("id", Operator.GT, "rec_003")
            .filter("category", Operator.EQ, "even")
            .sort_by("id", "asc")
        )
        results = db.search(query)
        assert len(results) == 3
        ...
        query = (
            Query()
            .filter("id", Operator.BETWEEN, ["rec_002", "rec_007"])
            .filter("value", Operator.GTE, 40)
            .sort_by("id", "asc")
        )
        results = db.search(query)
    ...
    def test_id_field_with_pagination(self, db):
        ...
        query = (
            Query().filter("id", Operator.GTE, "rec_003").sort_by("id", "asc").limit(3).offset(2)
        )
        results = db.search(query)
        assert len(results) == 3
