...
class TestSchema:
    ...
    def test_simple_schema(self):
        ...
        # Valid record
        record = Record({"username": "john_doe", "age": 30, "email": "john@example.com"})
        result = schema.validate(record)
        assert result.valid

        # Missing required field
        record = Record({"age": 30, "email": "john@example.com"})
        result = schema.validate(record)
        assert not result.valid
        assert "username" in str(result.errors)

        # Invalid constraint
        record = Record({"username": "john_doe", "age": 200, "email": "john@example.com"})
        result = schema.validate(record)
        assert not result.valid
    ...
    def test_strict_mode(self):
        ...
        record = Record({"allowed_field": "value", "unknown_field": "should fail"})
...
class TestCoercer:
    ...
    def test_coerce_many(self):
        ...
        values = {"age": "25", "height": "5.9", "active": "true"}

        types = {"age": int, "height": float, "active": bool}
...
class TestIntegration:
    ...
    def test_schema_with_coercion(self):
        ...
        record = Record({"age": "25", "score": "98.5", "active": "yes"})
    ...
    def test_complex_validation_scenario(self):
        ...
        schema.field(
            ...
            FieldType.STRING,
            required=True,
            constraints=[Length(min=3, max=20), Pattern(r"^[a-zA-Z0-9_]+$"), Unique("username")],
        )
        ...
        # Age: optional, 13-120
        schema.field("age", FieldType.INTEGER, constraints=[Range(min=13, max=120)])
        ...
        schema.field(
            ...
            FieldType.BOOLEAN,
            required=True,
            constraints=[Custom(lambda x: x is True, "Terms must be accepted")],
        )
        ...
        users = [
            ...
            Record({"username": "jane_smith", "email": "jane@example.org", "terms_accepted": True}),
        ]
