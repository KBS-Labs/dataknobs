...
class TestVectorIntegration:
    ...
    def test_create_memory_vector_store(self, factory):
        ...
        store = factory.create(backend="memory", dimensions=128, metric="cosine")
    ...
    async def test_end_to_end_vector_workflow(self, factory):
        ...
        store = factory.create(backend="memory", dimensions=64, metric="cosine")
        ...
        for i in range(10):
            ...
            await store.add_vectors(
                vectors=[vec], ids=[f"vec_{i}"], metadata=[{"index": i, "category": "test"}]
            )
        ...
        results = await store.search(query_vector=query_vec, k=5)
    ...
    async def test_vector_query_integration(self, factory):
        ...
        # Create a memory vector store
        store = factory.create(backend="memory", dimensions=128, metric="euclidean")
    ...
    async def test_hybrid_search_workflow(self, factory):
        ...
        for i in range(30):
            ...
            await store.add_vectors(
                vectors=[vec],
                ids=[f"item_{i}"],
                metadata=[{"title": f"Item {i}", "category": categories[i % 3], "views": i * 100}],
            )
    ...
    async def test_cross_backend_compatibility(self, factory):
        ...
        backends_to_test.append(("memory", {"dimensions": 64, "metric": "cosine"}))
