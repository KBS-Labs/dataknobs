...
def test_sync_vector_search(elasticsearch_test_index):
    ...
    try:
        # Create vector index
        db.create_vector_index(vector_field="embedding", dimensions=8, metric=DistanceMetric.COSINE)
        ...
        results = db.vector_search(
            query_vector=vec1, vector_field="embedding", k=3, metric=DistanceMetric.COSINE
        )
