...
class TestS3Backend:
    ...
    def test_create_and_read(self, mock_s3_backend):
        ...
        # Create a record
        record = Record({"name": "John Doe", "age": 30, "email": "john@example.com"})
    ...
    def test_search_with_projection(self, mock_s3_backend):
        ...
        record = Record({"name": "Alice", "age": 25, "email": "alice@example.com", "city": "NYC"})
        db.create(record)
    ...
    def test_batch_create(self, mock_s3_backend):
        ...
        # Create multiple records
        records = [Record({"name": f"User{i}", "index": i}) for i in range(10)]
    ...
    def test_metadata_as_tags(self, mock_s3_backend):
        ...
        record = Record(data={"name": "Test"}, metadata={"version": "1.0", "author": "tester"})
        record_id = db.create(record)
    ...
    def test_concurrent_operations(self, mock_s3_backend):
        ...
        records = [Record({"index": i, "data": f"record_{i}"}) for i in range(20)]
...
class TestS3Configuration:
    ...
    def test_from_config_method(self):
        ...
        with mock_aws():
            config = {"bucket": "test-bucket", "prefix": "data/", "region": "us-west-2"}
...
class TestS3RegionFallback:
    ...
    def test_bucket_create_uses_resolved_region_when_region_unset(self, monkeypatch):
        ...
        with mock_aws():
            ...
            location = db.s3_client.get_bucket_location(Bucket="fallback-bucket")[
                "LocationConstraint"
            ]
            assert location == "eu-west-1"
            db.close()

    def test_bucket_create_no_location_constraint_for_us_east_1(self, monkeypatch):
        ...
        with mock_aws():
            ...
            location = db.s3_client.get_bucket_location(Bucket="us-east-1-bucket")[
                "LocationConstraint"
            ]
