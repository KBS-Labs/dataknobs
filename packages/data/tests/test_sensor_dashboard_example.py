...
class TestSensorDashboardSync:
    ...
    def test_batch_reading_ingestion(self):
        ...
        readings = self.generator.generate_normal_readings(
            sensor.sensor_id, sensor.location, self.start_time, count=50
        )
    ...
    def test_streaming_ingestion(self):
        ...
        # Generate readings
        readings = self.generator.generate_normal_readings(
            sensor.sensor_id, sensor.location, self.start_time, count=100
        )
    ...
    def test_pandas_integration(self):
        ...
        readings = self.generator.generate_normal_readings(
            sensor.sensor_id, sensor.location, self.start_time, count=20
        )
    ...
    def test_anomaly_detection(self):
        ...
        normal = self.generator.generate_normal_readings(
            sensor.sensor_id, sensor.location, self.start_time, count=10
        )
        anomalous = self.generator.generate_anomalous_readings(
            sensor.sensor_id, sensor.location, self.start_time + timedelta(hours=1), count=5
        )
        ...
        anomalies = self.dashboard.detect_anomalies(
            all_readings, temp_threshold=(10, 35), humidity_threshold=(20, 80)
        )
    ...
    def test_duplicate_handling(self):
        ...
        with_duplicates = self.generator.generate_duplicate_readings(
            base_readings, duplicate_rate=0.3
        )
    ...
    def test_missing_data_interpolation(self):
        ...
        readings = self.generator.generate_missing_data_readings(
            sensor.sensor_id, sensor.location, self.start_time, count=50, gap_probability=0.2
        )
    ...
    def test_batch_failure_recovery(self):
        ...
        readings, expected_failures = self.generator.generate_batch_failure_scenario(
            sensor.sensor_id, sensor.location, self.start_time, count=30
        )
        ...
        assert stored_count == 24, (
            f"Expected 24 readings, got {stored_count}. Total sensor_readings: {len(sensor_readings)}"
        )
    ...
    def test_file_database_integration(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Add readings
            readings = self.generator.generate_normal_readings(
                self.sensors[0].sensor_id, self.sensors[0].location, self.start_time, count=10
            )
            ...
            # Verify persistence
            recent = file_dashboard.get_recent_readings(self.sensors[0].sensor_id, limit=5)
            assert len(recent) == 5
...
class TestSensorDashboardAsync:
    ...
    async def test_async_generator_processing(self):
        ...
        test_readings = self.generator.generate_normal_readings(
            sensor.sensor_id, sensor.location, self.start_time, count=5
        )
    ...
    async def test_async_file_database(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            readings = self.generator.generate_normal_readings(
                sensor.sensor_id, sensor.location, self.start_time, count=15
            )
...
class TestSensorDashboardEdgeCases:
    ...
    def test_out_of_order_processing(self):
        ...
        disordered = self.generator.generate_out_of_order_readings(ordered, disorder_rate=0.3)
    ...
    def test_concurrent_modifications(self):
        ...
        readings2 = self.generator.generate_normal_readings(
            sensor.sensor_id, sensor.location, self.start_time + timedelta(hours=1), count=10
        )
