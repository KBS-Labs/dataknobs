...
class TestSyncSQLiteDatabase:
    ...
    def test_batch_operations(self, memory_db):
        ...
        # Batch create
        records = [Record(data={"name": f"User{i}", "value": i}) for i in range(5)]
        ...
        # Batch update
        updates = [(ids[i], Record(data={"name": f"Updated{i}", "value": i * 2})) for i in range(3)]
    ...
    def test_journal_mode_wal(self, tmp_path):
        ...
        db = SyncSQLiteDatabase({"path": str(db_path), "journal_mode": "WAL"})
        db.connect()
...
class TestAsyncSQLiteDatabase:
    ...
    async def test_batch_operations_async(self):
        ...
        try:
            ...
            # Batch update
            updates = [
                (ids[i], Record(data={"name": f"Updated{i}", "value": i * 2})) for i in range(3)
            ]
...
class TestSQLiteSpecificFeatures:
    ...
    def test_json_validation(self, tmp_path):
        ...
        try:
            ...
            record = Record(data={"name": "Test", "nested": {"key": "value", "array": [1, 2, 3]}})
            id = db.create(record)
