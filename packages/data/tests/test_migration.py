...
class TestTransformer:
    ...
    def test_fluent_api(self):
        ...
        record = Record(
            {"id": 123, "username": "john", "password": "secret", "email": "john@example.com"}
        )
...
class TestMigrator:
    ...
    def test_simple_migration(self):
        ...
        progress = migrator.migrate(
            source=source, target=target, transform=transformer, batch_size=3
        )
    ...
    def test_migration_with_errors(self):
        ...
        migrator = Migrator()
        progress = migrator.migrate(
            source=source, target=target, transform=transformer, on_error=handle_error
        )
    ...
    def test_migration_with_filter(self):
        ...
        source = MemoryDatabase()
        for i in range(20):
            record = Record({"id": i, "category": "A" if i % 2 == 0 else "B", "value": i})
            source.create(record)
        ...
        migrator = Migrator()
        progress = migrator.migrate(source=source, target=target, query=query)
    ...
    def test_migration_with_progress_callback(self):
        ...
        def on_progress(progress):
            progress_updates.append({"processed": progress.processed, "percent": progress.percent})

        migrator = Migrator()
        progress = migrator.migrate(
            source=source, target=target, batch_size=5, on_progress=on_progress
        )
...
class TestIntegrationScenarios:
    ...
    def test_schema_evolution_migration(self):
        ...
        for i in range(5):
            record = Record(
                {"username": f"user_{i}", "email": f"user_{i}@example.com", "created": "2023-01-01"}
            )
        ...
        migration.add(TransformField("username", lambda x: split_username(x)["first"]))
        ...
        progress = migrator.migrate(source=source, target=target, transform=migration)
