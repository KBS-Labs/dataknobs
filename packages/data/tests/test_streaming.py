...
class TestStreamConfig:
    ...
    def test_custom_config(self):
        ...
        config = StreamConfig(batch_size=500, prefetch=5, timeout=30.0, on_error=error_handler)
    ...
    def test_on_conflict_from_dict_string(self):
        ...
        """
        assert (
            StreamConfig.from_dict({"on_conflict": "upsert"}).on_conflict is ConflictPolicy.UPSERT
        )

    def test_on_conflict_roundtrip(self):
        """on_conflict (an enum member) survives a to_dict -> from_dict round-trip."""
        assert_structured_config_roundtrip(StreamConfig(on_conflict=ConflictPolicy.SKIP))
...
class TestStreamResult:
    ...
    def test_merge_results(self):
        ...
        result1 = StreamResult(total_processed=50, successful=45, failed=5, duration=10.0)
        result1.add_error("rec1", ValueError("Error 1"))

        result2 = StreamResult(total_processed=30, successful=28, failed=2, duration=5.0)
    ...
    def test_string_representation(self):
        ...
        result = StreamResult(total_processed=100, successful=95, failed=5, duration=12.5)
...
class TestStreamProcessor:
    ...
    def test_filter_stream(self):
        ...
        # Filter even values
        filtered = list(
            StreamProcessor.filter_stream(iter(records), lambda r: r.get_value("value") % 2 == 0)
        )
    ...
    def test_transform_stream(self):
        ...
        transformed = list(StreamProcessor.transform_stream(iter(records), transform))
    ...
    async def test_async_batch_iterator(self):
        ...
        batches = []
        async for batch in StreamProcessor.async_batch_iterator(record_generator(), batch_size=3):
    ...
    async def test_async_filter_stream(self):
        ...
        async for record in StreamProcessor.async_filter_stream(
            record_generator(), lambda r: r.get_value("value") % 2 == 0
        ):
    ...
    async def test_async_transform_stream(self):
        ...
        transformed = []
        async for record in StreamProcessor.async_transform_stream(record_generator(), transform):
...
class TestSyncMemoryDatabaseStreaming:
    ...
    def setup_method(self):
        ...
        for i in range(100):
            record = Record(
                id=str(i).zfill(3)
            )  # Use zero-padded IDs for consistent string comparison
    ...
    def test_stream_transform(self):
        ...
        config = StreamConfig(batch_size=10)
        transformed = list(self.db.stream_transform(transform=double_values, config=config))
    ...
    def test_stream_transform_with_filter(self):
        ...
        transformed = list(
            self.db.stream_transform(query=query, transform=filter_and_double, config=config)
        )
...
class TestAsyncMemoryDatabaseStreaming:
    ...
    async def test_stream_transform(self):
        ...
        config = StreamConfig(batch_size=10)
        transformed = []
        async for record in self.db.stream_transform(transform=halve_values, config=config):
            transformed.append(record)
    ...
    async def test_error_handling_with_callback(self):
        ...
        config = StreamConfig(batch_size=3, on_error=error_handler)
...
class TestStreamingIntegration:
    ...
    def test_sync_memory_to_memory_migration(self):
        ...
        config = StreamConfig(batch_size=10)
        result = target.stream_write(source.stream_read(config=config), config=config)
    ...
    def test_filtered_migration(self):
        ...
        result = target.stream_write(source.stream_read(query, config), config)
    ...
    def test_transformed_migration(self):
        ...
        result = target.stream_write(
            source.stream_transform(transform=transform, config=config), config
        )
    ...
    async def test_async_memory_to_memory_migration(self):
        ...
        result = await target.stream_write(source.stream_read(config=config), config=config)
...
class TestBatchShortfallAccounting:
    ...
    def test_partial_batch_counts_shortfall_as_failed_and_quits(self):
        ...
        assert result.successful + result.failed + result.skipped == result.total_processed
        assert len(result.errors) == 1
