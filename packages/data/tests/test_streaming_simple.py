...
class TestAsyncV2Streaming:
    ...
    async def test_streaming_with_transformer(self):
        ...
        for i in range(1, 6):
            record = Record(
                {"old_id": i, "old_name": f"item_{i}", "temp": "remove_me", "keep": f"data_{i}"}
            )
            await db.create(record)
...
class TestSyncV2Streaming:
    ...
    def test_sync_streaming_with_migration(self):
        ...
        # Create test records
        for i in range(1, 21):
            record = Record({"user_id": i, "username": f"user_{i}", "score": i * 5})
            db.create(record)
...
class TestStreamingIntegration:
    ...
    async def test_migration_chain(self):
        ...
        # Create test data
        for i in range(1, 11):
            record = Record({"v1_id": i, "v1_data": f"data_{i}", "deprecated": "old_value"})
            await db.create(record)
