...
class TestRangeOperators:
    ...
    def test_datetime_between(self):
        ...
        query = Query(
            filters=[Filter("timestamp", Operator.BETWEEN, (start.isoformat(), end.isoformat()))]
        )
        results = db.search(query)
        assert len(results) == 2
        assert set(r.id for r in results) == {"2", "3"}
    ...
    def test_nested_field_between(self):
        ...
        records = [
            Record(id="1", data={"metrics": {"cpu": 25, "memory": 40}}, metadata={"priority": 1}),
            Record(id="2", data={"metrics": {"cpu": 50, "memory": 60}}, metadata={"priority": 2}),
            Record(id="3", data={"metrics": {"cpu": 75, "memory": 80}}, metadata={"priority": 3}),
            Record(id="4", data={"metrics": {"cpu": 90, "memory": 95}}, metadata={"priority": 4}),
        ]
    ...
    def test_combined_filters_with_between(self):
        ...
        records = [
            ...
            Record(
                id="2",
                data={"temperature": 25.0, "timestamp": base_time.isoformat()},
                metadata={"sensor": "A"},
            ),
            Record(
                id="3",
