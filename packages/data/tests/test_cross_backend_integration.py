...
class TestSyncCrossBackendStreaming:
    ...
    def test_transformed_streaming(self):
        ...
        source_stream = self.memory_db.stream_transform(transform=transform_record, config=config)
        result = self.file_db.stream_write(source_stream, config)
...
class TestSyncCrossBackendMigration:
    ...
    def test_simple_migration(self):
        ...
        migrator = Migrator()
        progress = migrator.migrate(source=self.source_db, target=self.target_db, batch_size=10)
    ...
    def test_migration_with_schema_change(self):
        ...
        progress = migrator.migrate(
            source=self.source_db, target=self.target_db, transform=migration, batch_size=10
        )
    ...
    def test_migration_with_complex_transform(self):
        ...
        transformer.map(
            "age",
            "age_group",
            lambda age: "young" if age < 30 else "middle" if age < 50 else "senior",
        )
        transformer.map(
            "email",
            "email_domain",
            lambda email: email.split("@")[1] if "@" in email else "unknown",
        )
        ...
        migrator = Migrator()
        progress = migrator.migrate(
            source=self.source_db, target=self.target_db, transform=transformer, batch_size=15
        )
    ...
    def test_migration_validation(self):
        ...
        # First migrate
        migrator = Migrator()
        progress = migrator.migrate(source=self.source_db, target=self.target_db)

        assert progress.succeeded == 50

        # Validate migration
        is_valid, issues = migrator.validate_migration(self.source_db, self.target_db)
...
class TestAsyncCrossBackendOperations:
    ...
    async def create_test_environment(self):
        ...
        for i in range(75):
            record = Record(
                {"value": i * 5, "status": "pending" if i % 3 == 0 else "active"},
                id=f"async_{i}",
            )
    ...
    async def test_async_filtered_migration(self):
        ...
        progress = await migrator.migrate_async(
            source=self.async_memory, target=self.async_file, query=query
        )
    ...
    async def test_async_transform_migration(self):
        ...
        migrator = Migrator()
        progress = await migrator.migrate_async(
            source=self.async_memory, target=self.async_file, transform=transformer
        )
...
class TestErrorHandlingAcrossBackends:
    ...
    def test_migration_with_mixed_errors(self):
        ...
        for i in range(15):
            record = Record(
                {"value": i, "type": "normal" if i % 3 != 0 else "special"},
                id=i,
            )
            source.create(record)
        ...
        migrator = Migrator()
        progress = migrator.migrate(
            source=source, target=target, transform=FailingTransformer(), on_error=error_handler
        )
