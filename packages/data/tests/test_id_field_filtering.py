...
class TestIdFieldFilteringAllBackends:
    ...
    def test_id_field_combined_with_data_filters(self, db):
        ...
        for i in range(10):
            record = Record(
                id=str(i), data={"value": i * 10, "category": "even" if i % 2 == 0 else "odd"}
            )
            db.create(record)
        ...
        query = (
            Query()
            .filter("id", Operator.BETWEEN, ["2", "7"])
    ...
    def test_id_field_with_pagination(self, db):
        ...
        for i in range(10):
            record = Record(
                id=str(i).zfill(2), data={"value": i}
            )  # Use zero-padded IDs for consistent string sorting
            db.create(record)
