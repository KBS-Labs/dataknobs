...
class TestElasticsearchRangeOperators:
    ...
    def test_nested_field_between_elasticsearch(self, es_db):
        ...
        records = [
            Record(id="1", data={"metrics": {"cpu": 25}}, metadata={"priority": 1}),
            Record(id="2", data={"metrics": {"cpu": 50}}, metadata={"priority": 2}),
            Record(id="3", data={"metrics": {"cpu": 75}}, metadata={"priority": 3}),
            Record(id="4", data={"metrics": {"cpu": 90}}, metadata={"priority": 4}),
        ]
    ...
    async def test_async_elasticsearch_between(self):
        ...
        db = AsyncElasticsearchDatabase(
            config={"host": host, "port": port, "index": "test_async_range_index", "refresh": True}
        )
...
class TestCrossBackendConsistency:
    ...
    def test_data(self):
        ...
        return [
            ...
            Record(
                id="2", data={"price": 150.0, "name": "Beta", "timestamp": base_time.isoformat()}
            ),
            Record(
                id="3",
                data={
