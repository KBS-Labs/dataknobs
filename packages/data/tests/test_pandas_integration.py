...
class TestDataFrameConverter:
    ...
    def test_dataframe_to_records_with_nulls(self):
        ...
        df = pd.DataFrame(
            {"id": [1, 2, 3], "name": ["Alice", None, "Charlie"], "value": [10.5, np.nan, 20.0]}
        )
...
class TestBatchOperations:
    ...
    def test_bulk_insert_dataframe(self):
        ...
        result = self.batch_ops.bulk_insert_dataframe(self.df, config=config)
    ...
    def test_update_from_dataframe(self):
        ...
        update_df = pd.DataFrame(
            {"record_id": record_ids, "value": [100, 200, 300], "category": ["NEW", "NEW", "NEW"]}
        )

        config = BatchConfig(chunk_size=2)
        result = self.batch_ops.update_from_dataframe(
            update_df, id_column="record_id", config=config
        )
    ...
    def test_parallel_bulk_insert(self):
        ...
        config = BatchConfig(chunk_size=25, parallel=True, max_workers=2)
    ...
    def test_aggregate(self):
        ...
        # Insert test data
        for i in range(10):
            record = Record({"category": "A" if i < 5 else "B", "value": i * 10, "count": i})
            self.db.create(record)
        ...
        agg_result = self.batch_ops.aggregate(query, aggregations={"value": "sum", "count": "mean"})
        ...
        grouped_result = self.batch_ops.aggregate(
            query, aggregations={"value": "sum", "count": "count"}, group_by=["category"]
        )
    ...
    def test_transform_and_save(self):
        ...
        result = self.batch_ops.transform_and_save(
            Query(), transformer=double_values, config=BatchConfig(chunk_size=2)
        )
    ...
    def test_export_to_csv(self, tmp_path):
        ...
        self.batch_ops.export_to_csv(Query(), str(csv_path), index=False)
    ...
    def test_export_to_parquet(self, tmp_path):
        ...
        self.batch_ops.export_to_parquet(Query(), str(parquet_path))
