...
class TestDatabaseFactory:
    ...
    def test_create_s3_backend(self, mock_boto_client):
        ...
        db = factory.create(backend="s3", bucket="test-bucket", prefix="test/", region="us-west-2")
...
class TestFactoryWithConfig:
    ...
    def test_factory_registration(self):
        ...
        config.load(
            {"databases": [{"name": "test_db", "factory": "database", "backend": "memory"}]}
        )
    ...
    def test_multiple_backends_from_factory(self):
        ...
        try:
            config.load(
                {
                    "databases": [
                        {"name": "memory_db", "factory": "database", "backend": "memory"},
                        {
                            "name": "file_db",
                            "factory": "database",
                            "backend": "file",
