...
class MultiBackendVectorExample:
        ...
        for i, item in enumerate(self.test_data):
            ...
            record = Record(
                data={
                    ...
                    "embedding": VectorField(name="embedding", value=vec, dimensions=3),
                },
                metadata={"index": i},
            )
    ...
    def run_sync_file_example(self, filepath: Optional[str] = None) -> List[Any]:
        ...
        try:
            ...
            factory = DatabaseFactory()
            db = factory.create(
                backend="file", path=filepath, vector_enabled=True, vector_metric="euclidean"
            )
    ...
    async def run_async_memory_example(self) -> List[Any]:
        ...
        db = factory.create(backend="memory", vector_enabled=True, vector_metric="dot_product")
    ...
    async def run_async_file_with_filter(self, filepath: Optional[str] = None) -> List[Any]:
        ...
        try:
            ...
            factory = AsyncDatabaseFactory()
            db = factory.create(
                backend="file", path=filepath, vector_enabled=True, vector_metric="cosine"
            )
