...
class HybridSearchDemo:
    ...
    async def text_search(self, query: str, limit: int = 5) -> List[SearchResult]:
        ...
        # `like` with wildcards, not `contains`: there is no `contains`
        # operator, and the fluent builder used to read an unknown spelling as
        # equality --- so this asked for content *equal to* the query string.
        results = await self.db.search(Query().filter("content", "like", f"%{query}%").limit(limit))
    ...
    async def vector_search(self, query: str, limit: int = 5) -> List[SearchResult]:
        ...
        results = await self.db.vector_search(
            query_vector=query_embedding, k=limit, vector_field="embedding"
        )

        return [
            SearchResult(record=r.record, score=r.score, method="vector", rank=i)
            for i, r in enumerate(results)
        ]
    ...
    async def hybrid_search(
        ...
        return [
            SearchResult(record=all_records[record_id], score=score, method="hybrid", rank=i)
    ...
    async def reciprocal_rank_fusion(
        ...
        return [
            SearchResult(record=all_records[record_id], score=score, method="rrf", rank=i)
            for i, (record_id, score) in enumerate(sorted_results)
        ]
