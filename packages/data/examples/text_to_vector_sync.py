...
async def main():
    ...
    results = await sync.synchronizer.sync_all(
        ...
        progress_callback=lambda status: print(
            f"  Progress: {status.processed_records}/{status.total_records} records processed"
        ),
    )
    ...
    results = await db.vector_search(query_vector=query_embedding, k=3, vector_field="embedding")
