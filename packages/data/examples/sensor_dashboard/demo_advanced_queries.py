...
def demo_range_operators(dashboard: SensorDashboard):
    ...
    optimal = dashboard.get_optimal_conditions(
        temp_range=(18, 25), humidity_range=(40, 60), min_battery=50.0
    )
    ...
    anomalies = dashboard.get_anomalous_readings_query(temp_range=(10, 35), humidity_range=(20, 80))
    print(f"   Found {len(anomalies)} anomalous readings")
    for reading in anomalies[:3]:  # Show first 3
        print(
            f"   - Sensor {reading.sensor_id}: Temp={reading.temperature}°C, "
...
def demo_query_builder(dashboard: SensorDashboard):
    ...
    hot_dry = (
        QueryBuilder().where("temperature", Operator.GT, 25).where("humidity", Operator.LT, 50)
    )

    # Low battery in warehouse
    low_battery_warehouse = (
        QueryBuilder().where("battery", Operator.LT, 20).where("location", Operator.EQ, "warehouse")
    )

    # Combine with OR
    builder.and_(QueryBuilder().or_(hot_dry, low_battery_warehouse))
