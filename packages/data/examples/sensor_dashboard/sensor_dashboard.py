...
class SensorDashboard:
    ...
    def stream_readings(self, readings: List[SensorReading], batch_size: int = 10) -> StreamResult:
    ...
    def stream_readings_with_tracking(
        self, readings: List[SensorReading], batch_size: int = 10, track_failures: bool = True
    ) -> StreamResult:
        ...
        config = StreamConfig(
            batch_size=batch_size, on_error=error_handler if track_failures else None
        )
    ...
    def get_all_sensors(self) -> List[SensorInfo]:
        ...
        query = Query(filters=[Filter("metadata.type", Operator.EQ, "sensor_info")])
    ...
    def detect_anomalies(
        ...
        humidity_threshold: tuple = (20, 80),
    ) -> List[SensorReading]:
    ...
    def get_anomalous_readings_query(
        self, temp_range: tuple = (10, 35), humidity_range: tuple = (20, 80)
    ) -> List[SensorReading]:
    ...
    def get_critical_sensors(
        self, battery_threshold: float = 20.0, locations: List[str] = None
    ) -> List[SensorReading]:
        ...
        builder.and_(QueryBuilder().or_(critical_location, extreme_temp))
    ...
    def get_sensors_by_multiple_locations(self, locations: List[str]) -> List[SensorReading]:
        ...
        query = (
            Query().filter("metadata.type", Operator.EQ, "sensor_reading").or_(*location_filters)
        )
...
class AsyncSensorDashboard:
    ...
    async def stream_readings(
        self, readings: List[SensorReading], batch_size: int = 10
    ) -> StreamResult:
        """Stream readings with configurable batch size."""
    ...
    async def stream_readings_with_tracking(
        self, readings: List[SensorReading], batch_size: int = 10
    ) -> StreamResult:
