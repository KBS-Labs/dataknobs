...
from dataknobs_data import Record, Query, database_factory, DatabaseFactory
...
def demonstrate_crud_operations(db, db_name):
    ...
    records = [
        ...
        Record(
            {"name": "Bob Smith", "age": 35, "department": "Sales", "salary": 75000, "active": True}
        ),
        Record(
            {
                "name": "Charlie Brown",
    ...
    query = Query().filter("salary", ">=", 80000).sort("salary", "DESC").limit(2)
    results = db.search(query)
