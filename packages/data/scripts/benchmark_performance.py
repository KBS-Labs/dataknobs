...
class ValidationBenchmark(Benchmark):
    ...
    def setup(self):
        if self.complexity == "simple":
            ...
            self.record = Record(
                {"id": random.randint(1, 1000), "name": f"item_{random.randint(1, 100)}"}
            )
            ...
            self.schema.field(
                ...
                "STRING",
                required=True,
                constraints=[Length(min=3, max=20), Pattern(r"^[a-zA-Z0-9_]+$")],
            )
            self.schema.field(
                "email",
                "STRING",
...
class UniqueConstraintBenchmark(Benchmark):
    ...
    def setup(self):
        ...
        for i in range(100):
            ...
            self.records.append(Record({"id": id_val, "username": f"user_{id_val}"}))
...
class DatabaseMigrationBenchmark(Benchmark):
    ...
    def execute(self):
        ...
        progress = migrator.migrate(
            source=self.source, target=self.target, transform=self.transformer, batch_size=100
        )
