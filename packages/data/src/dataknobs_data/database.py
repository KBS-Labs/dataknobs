...
class Database(ABC):
    ...
    @classmethod
    def create(cls, backend: str, config: Optional[Dict[str, Any]] = None) -> 'Database':
...
class SyncDatabase(ABC):
    ...
    @classmethod
    def create(cls, backend: str, config: Optional[Dict[str, Any]] = None) -> 'SyncDatabase':
        ...
        if not backend_class:
            raise ValueError(f"Unknown backend: {backend}. Available: {list(SYNC_BACKEND_REGISTRY.keys())}")
